...
const EXTERNAL_CARLOS: ExternalParticipant = { name: 'Carlos Martínez', email: 'carlos.martinez@clientealfa.example' }
const EXTERNAL_ELENA: ExternalParticipant = { name: 'Elena Vidal', email: 'elena.vidal@nubemx.example' }
const EXTERNAL_RICARDO: ExternalParticipant = { name: 'Ricardo Ruiz', email: 'ricardo@despachoruiz.example' }
const EXTERNAL_INVERSIONISTA: ExternalParticipant = { name: 'Tomás Aguirre', email: 'taguirre@fondogamma.example' }
...
const MEETINGS: MeetingDef[] = [
  {
    ...
    segments: [
      ['andres', 'Buenos días a todos. El objetivo de hoy es revisar el estatus del contrato con Cliente Alfa y cerrar los pendientes antes de la firma.'],
      ['direccion', 'Perfecto. Carlos, gracias por acompañarnos. ¿Cómo va la revisión de su lado?'],
      ['Carlos Martínez', 'Gracias, Lucía. Nuestro comité ya aprobó las condiciones comerciales. Sólo nos falta formalizar la intención por escrito.'],
      ['andres', '¿Tenemos fecha para eso?'],
      ['andres', 'Entonces quedamos así: Carlos enviará la carta de intención el próximo martes y nosotros preparamos el cronograma.'],
      ['Carlos Martínez', 'Correcto, el martes la tienen en su correo. Ya la tengo en borrador con nuestro director.'],
      ['gestora', 'Lo anoto. ¿La carta va dirigida a Lucía o a Andrés?'],
      ['direccion', 'A Andrés, que lleva la relación. Yo firmo después.'],
      ['andres', 'Ahora, el anexo de penalizaciones. Lisa nos comentó que hay dos cláusulas que le preocupan.'],
      ['direccion', 'Sí, la de retraso por causas del cliente y la de terminación anticipada.'],
      ['Carlos Martínez', 'De nuestro lado no hay problema en ajustar los porcentajes si se justifican.'],
      ['andres', 'Quedamos en que Jurídico revisa el anexo de penalizaciones antes del viernes y les manda comentarios.'],
      ...
      ['Carlos Martínez', 'Lo del presupuesto de licencias ya quedó cerrado, lo mandé ayer a Héctor con el desglose.'],
      ['andres', 'Perfecto, entonces esa tarea la podemos dar por concluida en cuanto Héctor confirme.'],
      ['gestora', 'Héctor no está en la llamada; le pido que lo valide hoy.'],
      ['direccion', 'Siguiente punto: el cronograma de implementación.'],
      ['andres', 'Yo preparo una propuesta de cronograma de implementación para la próxima semana, con hitos por fase.'],
    ...
    summary: {
      ...
      topics: ['Carta de intención', 'Anexo de penalizaciones', 'Presupuesto de licencias', 'Cronograma de implementación'],
      risks: ['Tiempo de respuesta del área legal del cliente puede retrasar la firma.'],
      openQuestions: ['¿Héctor confirmó la recepción del presupuesto de licencias?', '¿La firma se mantiene para la semana del quince?'],
    },
    decisions: [
      {
        description: 'La carta de intención se dirigirá a Andrés Escandón, quien lleva la relación con Cliente Alfa.',
      ...
      {
        description: 'Se ajustarán los porcentajes de penalización si se justifican; Jurídico envía comentarios antes del viernes.',
  ...
  },
  {
    ...
    segments: [
      ['direccion', 'Arrancamos el comité. Primero, resultados de agosto y después los objetivos del trimestre.'],
      ['finanzas', 'Cerramos agosto con ingresos ligeramente por arriba del plan; el detalle lo presento el jueves.'],
      ['direccion', 'Necesito que cada área defina sus OKRs del cuarto trimestre. Andrés coordina la plantilla.'],
      ['andres', 'De acuerdo, la circulo hoy. Sobre Alfa: la carta de intención sigue pendiente de recibir.'],
      ...
      ['direccion', 'También necesitamos definir el SLA de respuesta para tickets del portal; Iván ya lo tiene en su lista.'],
    ...
    summary: {
      ...
      risks: ['La carta de intención de Cliente Alfa se ha mencionado en varias reuniones sin avance.'],
  ...
  },
  {
    ...
    segments: [
      ['juridico', 'Revisé el borrador del contrato que enviamos; el cliente ya devolvió comentarios menores.'],
      ['andres', 'La cláusula de exclusividad era la que más preocupaba. ¿Ya tenemos dictamen?'],
      ['juridico', 'Sí, el dictamen sobre exclusividad quedó listo y está en la carpeta compartida.'],
      ['direccion', 'Bien. Lo que sigue es el anexo de penalizaciones, que aún no se ha negociado.'],
      ['juridico', 'También necesito actualizar la política de firmas electrónicas; estoy esperando al proveedor.'],
      ['andres', 'Y recordar que seguimos sin recibir la carta de intención de Alfa.'],
    ],
    summary: {
      executive: ['Dictamen sobre exclusividad concluido.', 'El anexo de penalizaciones sigue pendiente de negociación.'],
      detailed:
  ...
  },
  {
    ...
    segments: [
      ['andres', 'Bienvenidos al kickoff de Plataforma Beta. Elena, gracias por acompañarnos de parte de Nube MX.'],
      ['Elena Vidal', 'Con gusto. Nosotros entregamos las credenciales del ambiente de pruebas en cuanto se firme el anexo de soporte.'],
      ['operaciones', 'Yo configuro el ambiente de pruebas en cuanto tenga esas credenciales.'],
      ['andres', 'Rodrigo, también necesitamos documentar los requerimientos de integración con el CRM.'],
    ...
    summary: {
      executive: ['Nube MX entregará credenciales del ambiente de pruebas tras firmar el anexo de soporte.', 'Rodrigo documentará requerimientos de integración con CRM.'],
      detailed:
  ...
  },
  {
    ...
    segments: [
      ...
      ['direccion', 'Quiero que la campaña digital del Q4 esté diseñada antes de que termine el mes.'],
      ['ventas', 'Va. También propongo actualizar el pipeline en el CRM cada semana, los lunes.'],
      ['capital', 'Desde captación podemos compartir contactos de inversionistas que también son prospectos.'],
      ['ventas', 'Igual habría que cotizar una agencia para producir el video de la campaña, si el presupuesto alcanza.'],
      ['direccion', 'Cotícenlo y lo vemos.'],
    ],
    summary: {
      executive: ['Doce oportunidades activas en el pipeline del Q4.', 'La campaña digital debe quedar diseñada antes de fin de mes.'],
      detailed:
  ...
  },
  {
    ...
    segments: [
      ['finanzas', 'Las cuentas bancarias de agosto ya están conciliadas y los estados financieros se enviaron al despacho.'],
      ['Ricardo Ruiz', 'Los recibimos. Nosotros revisamos las deducciones fiscales de agosto y les regresamos observaciones la próxima semana.'],
      ['direccion', 'Necesito la presentación de resultados de agosto para el comité; Lucía la preparo yo con base en tus números.'],
      ['gestora', 'Se acordó que el despacho es responsable de las deducciones; lo registro como tarea externa.'],
    ...
    summary: {
      executive: ['Conciliación bancaria de agosto concluida.', 'El despacho revisará deducciones fiscales la próxima semana.'],
      detailed:
    ...
    decisions: [
      {
        description: 'El Despacho Contable Ruiz es responsable de la revisión de deducciones fiscales de agosto.',
  ...
  },
  {
    ...
    segments: [
      ['capital', 'Tenemos tres inversionistas interesados en Fondo Gamma. Necesito el deck listo para la primera ronda.'],
      ['direccion', 'El deck lo preparas tú con apoyo de Andrés en la parte operativa.'],
      ['capital', 'Después de la primera ronda agendamos una segunda con los que sigan interesados.'],
    ...
    summary: {
      executive: ['Tres inversionistas interesados en Fondo Gamma.', 'Comisión de éxito fijada en 2% para la ronda.'],
      detailed:
  ...
  },
  {
    ...
    segments: [
      ['servicio', 'Tenemos un backlog de quejas del portal sin responder; lo estoy atacando esta semana.'],
      ['ventas', 'Hay que implementar la encuesta NPS después de cada atención; creo que ya está casi lista.'],
      ['servicio', 'La encuesta NPS ya está en producción desde ayer, sólo falta revisar los primeros resultados.'],
      ['operaciones', 'Alguien del equipo de soporte tiene que capacitarse en el nuevo portal antes de la liberación.'],
      ['servicio', 'Sí, falta definir quién. También hay que definir el SLA de respuesta para tickets.'],
      ['ventas', 'Y quién dará mantenimiento al portal después; nadie lo tiene asignado.'],
    ],
    summary: {
      executive: ['Backlog de quejas del portal en atención.', 'Encuesta NPS reportada como ya en producción.'],
      detailed:
      ...
      openQuestions: ['¿Quién capacita al equipo de soporte?', '¿Quién será responsable del mantenimiento del portal?'],
    },
  },
  {
    key: 'norte',
    ...
    segments: [
      ['direccion', 'La expansión a Monterrey es prioridad del segundo semestre. Andrés evalúa locales comerciales.'],
      ...
      ['finanzas', 'La proyección financiera 2027 la puedo hacer, pero necesito los supuestos de renta del local.'],
    ...
    summary: {
      executive: ['Apertura objetivo en Monterrey: Q1 2027.', 'Proyección financiera bloqueada por falta de supuestos de renta.'],
      detailed:
      ...
      topics: ['Locales comerciales', 'Estudio de mercado', 'Proyección financiera', 'Permisos municipales'],
      risks: ['Ayuntamiento sin respuesta para permisos.', 'Proyección financiera depende de la selección del local.'],
      openQuestions: ['¿Cuándo responde el ayuntamiento?'],
    },
    decisions: [
      {
        description: 'La apertura objetivo del local de Monterrey será el primer trimestre de 2027.',
        decidedBy: 'Lucía Ferrer',
        evidenceSeq: [6],
        confidence: 0.92,
        status: DecisionStatus.CONFIRMED,
  ...
  },
  {
    ...
    segments: [
      ['andres', 'Gracias por el tiempo, Carlos. Queremos alinear los términos comerciales iniciales del contrato.'],
      ['Carlos Martínez', 'De acuerdo. Nosotros necesitamos que nos manden la carta de intención firmada de su lado y luego devolvemos la nuestra.'],
    ...
    summary: {
      executive: ['Se alinearon términos comerciales iniciales con Cliente Alfa.', 'Andrés enviará la carta de intención; Jurídico el borrador del contrato.'],
      detailed:
  ...
  },
  {
    ...
    segments: [
      ...
      ['Elena Vidal', 'Las entregamos en cuanto Jurídico nos regrese el contrato de soporte revisado.'],
      ['andres', 'Entonces Lisa revisa el contrato de soporte con Nube MX. Rodrigo, ¿y el módulo de reportes?'],
      ['operaciones', 'Estoy corrigiendo los errores de carga del módulo de reportes; es urgente para la demo.'],
      ['andres', 'Falta definir la fecha de liberación de la versión uno; yo diría a finales de la próxima semana, pero lo confirmo.'],
      ['Elena Vidal', 'Nos parece bien, avísennos.'],
    ],
    summary: {
      executive: ['Ambiente de pruebas bloqueado por credenciales pendientes de Nube MX.', 'Fecha de liberación v1 aún por confirmar.'],
      detailed:
      ...
      topics: ['Ambiente de pruebas', 'Contrato de soporte', 'Módulo de reportes', 'Fecha de liberación'],
...
export async function seedMeetings(db: PrismaClient, cat: Catalogs): Promise<MeetingsResult> {
  ...
  const counts = { meetings: 0, participants: 0, transcripts: 0, segments: 0, summaries: 0, decisions: 0, processingRuns: 0 }

  for (const m of MEETINGS) {
    const id = stableId(`meeting:${m.title}`)
    const startAt = m.startsInHours !== undefined ? hoursFromNow(m.startsInHours) : at(m.dayOffset, m.hour)
    ...
    const conferenceRecordId = ended && m.source !== MeetingSource.LEGACY_IMPORT ? `conferenceRecords/fake-${m.key}` : null
    const data = {
      title: m.title,
    ...
    await db.processingRun.upsert({ where: { id: runId }, create: { id: runId, ...runData }, update: runData })
    runs[m.key] = runId
    counts.processingRuns++

    // Transcript + segmentos.
    const lines = m.segments.map(([speaker, text]) => {
      const label = typeof speaker === 'string' && speaker in cat.userNames ? cat.userNames[speaker as UserKey] : speaker
      return `${label}: ${text}`
    })
    ...
    counts.transcripts++
    const secondsPerSegment = Math.max(20, Math.floor((m.durationMin * 60) / (m.segments.length + 1)))
    for (const [i, [speaker, text]] of m.segments.entries()) {
      ...
      await db.transcriptSegment.upsert({ where: { id: segId }, create: { id: segId, ...segData }, update: segData })
    ...
    if (m.summary) {
      ...
      const summaryData = {
        ...
        approvedAt: m.processing === MeetingProcessingStatus.COMPLETED ? addMinutes(startAt, m.durationMin + 60) : null,
        approvedByUserId: m.processing === MeetingProcessingStatus.COMPLETED ? cat.users['gestora'] : null,
      }
