...
export function confidenceBand(
  ...
  thresholds = { autoAccept: 0.9, proposal: 0.7 },
): ConfidenceBand | null {
...
export function ConfidenceIndicator({
  ...
  if (band === null) {
    return (
      <span
        ...
        title="Sin confianza registrada"
      >
        —
      </span>
    )
          ...
          {[0, 1, 2].map((i) => (
            <span
              ...
              className={cn(
                ...
                i < filled ? tone.split(' ')[0] : 'bg-paper-300',
              )}
            />
          ))}
        </span>
        {showLabel ? (
          <span className={cn('font-mono text-xs tabular', textTone)}>{pct}%</span>
        ) : null}
      </span>
    )
  }
  ...
  return (
    <span
      className={cn('inline-flex min-w-[88px] items-center gap-2', className)}
      title={title}
      aria-label={title}
    >
      ...
      {showLabel ? (
        <span className={cn('w-9 text-right font-mono text-xs tabular', textTone)}>{pct}%</span>
      ) : null}
    </span>
  )
}
