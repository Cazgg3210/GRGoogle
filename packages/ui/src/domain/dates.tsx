...
export function RelativeDate({ value, className, withTime }: { value: string | null | undefined; className?: string; withTime?: boolean }) {
...
export function DueDate({
  ...
  if (!value) {
    return (
      <span className={cn('inline-flex items-center gap-1 rounded-sm border border-dashed border-warning-400 bg-warning-50 px-1.5 py-0.5 text-[11px] font-semibold uppercase tracking-wider text-warning-800', className)}>
  ...
  return (
    <span
      className={cn(
        ...
        overdue ? 'font-semibold text-danger-700' : closed ? 'text-muted-foreground' : 'text-foreground',
        className,
      )}
      title={overdue ? 'Vencida' : undefined}
    >
      <time dateTime={value}>{formatDate(value)}</time>
      {hint ? <span className={cn('text-[11px]', overdue ? 'text-danger-600' : 'text-muted-foreground')}>{hint}</span> : null}
    </span>
  )
}
