...
describe('descubrimiento por Calendar', () => {
  it('crea reuniones, detecta host externo, cancela, aplica/bloquea auto-captura y encola safety net', async () => {
    ...
    expect(beta).toMatchObject({
      ...
      googleMeetingCode: 'qrs-tuvw-xyz',
    })
    ...
    expect(
      (await ctx.repos.meetings.listParticipants(alfa?.id ?? '')).some(
        (p) => p.email === 'carlos.martinez@cliente-alfa.example' && !p.isInternal,
      ),
    ).toBe(true)
    ...
    expect(pipeline).toMatchObject({
      ...
      lastErrorCode: DomainErrorCode.GOOGLE_CAPABILITY_BLOCKED,
    })
    ...
    ctx.google.calendar.addEvent({
      userEmail: u.andres.email,
      calendarEventId: 'cal-evt-alfa-001',
      title: 'Seguimiento contrato Cliente Alfa',
      organizerEmail: u.andres.email,
      attendees: [],
      startOffsetMinutes: -1500,
      durationMinutes: 60,
    ...
    expect((await ctx.repos.meetings.findById(alfa?.id ?? ''))?.status).toBe(
  ...
  it('token inválido fuerza sync completo y errores por usuario no abortan el loop', async () => {
    ...
    expect(res.errors).toEqual([])
    expect(
      (await ctx.repos.calendarCursors.find(u.andres.id, 'primary'))?.lastFullSyncAt,
    ).not.toBeNull()
  })
})
...
describe('suscripciones Workspace Events', () => {
  it('crea por usuario monitoreado, renueva cerca de expirar y registra errores sin abortar', async () => {
    ...
    expect(first).toMatchObject({
      users: 3,
      created: 2,
      errors: [{ userEmail: u.mariana.email, code: DomainErrorCode.GOOGLE_PERMISSION_DENIED }],
    })
...
describe('pipeline evento → artefactos → análisis', () => {
  it('conference.ended es idempotente y enlaza con la reunión descubierta por Calendar', async () => {
    ...
    const event = createFakeConferenceEndedEvent('abc-defg-hij', {
      subscribedUserEmail: u.andres.email,
      id: 'evt-1',
    })
    ...
    const meeting = await ctx.repos.meetings.findByConferenceRecordId(
      'conferenceRecords/fake-alfa-001',
    )
    ...
    const unknown = await processInboundGoogleEvent(ctx, {
      ...event,
      id: 'evt-2',
    ...
    expect(
      (await ctx.repos.inboundEvents.listRecent(10)).map((e) => e.processingStatus).sort(),
  ...
  it('crea la reunión desde el evento cuando no fue descubierta y procesa end-to-end (fixture Alfa → 3 items)', async () => {
    ...
    const opsEvent = createFakeConferenceEndedEvent('wxy-zabc-def', {
      subscribedUserEmail: u.mariana.email,
    })
    const ops = await processInboundGoogleEvent(ctx, opsEvent)
    expect(await ctx.repos.meetings.findById(ops.meetingId as string)).toMatchObject({
      source: 'WORKSPACE_EVENT',
      organizerEmail: u.mariana.email,
      organizerUserId: u.mariana.id,
      googleMeetingCode: 'wxy-zabc-def',
    ...
    expect((await ctx.repos.meetings.findById(meetingId))?.title).toBe(
      'Seguimiento contrato Cliente Alfa',
    )
    ...
    expect(artifacts).toMatchObject({
      ...
      processingStatus: MeetingProcessingStatus.INGESTED,
      enqueuedAnalysis: true,
    })
    const transcripts = await ctx.repos.transcripts.findByMeeting(meetingId)
    expect(transcripts.map((t) => t.sourceType).sort()).toEqual([
      'MEET_SMART_NOTES',
      'MEET_TRANSCRIPT',
    ])
    const segments = await ctx.repos.transcripts.listSegments(
      transcripts.find((t) => t.sourceType === 'MEET_TRANSCRIPT')?.id ?? '',
    )
    expect(segments.length).toBeGreaterThan(20)
    expect(segments[0]?.speakerLabel).toBe('Andrés Escandón')
    expect(segments[0]?.participantId).not.toBeNull()
    expect(
      (await ctx.repos.meetings.listParticipants(meetingId)).find(
        (p) => p.displayName === 'Lucía Ferrer',
      )?.internalUserId,
    ).toBe(u.lucia.id)
    ...
    expect(detail.processingRuns[0]).toMatchObject({
      success: true,
      provider: 'fake',
      promptVersion: 'v1',
    })
    ...
    await expect(
      getMeetingDetail(ctx, principalOf({ ...u.mariana, id: 'otro' }), meetingId),
  ...
  it('simulateMeetingEnded ejecuta todo el pipeline en modo FAKE', async () => {
    ...
    await expect(simulateMeetingEnded(ctx, principalOf(u.mariana), {})).rejects.toMatchObject({
  ...
  it('host externo sin artefactos accesibles → UNAVAILABLE_EXTERNAL_HOST y COMPLETED (safety net)', async () => {
    ...
    const ops = [...ctx.state.meetings.values()].find(
      (m) => m.title === 'Sincronización semanal Operaciones',
    )
    const r2 = await reconcileMissingEvents(ctx, { meetingId: ops?.id })
    expect(r2.linked).toBe(1)
    expect((await ctx.repos.meetings.findById(ops?.id ?? ''))?.googleConferenceRecordId).toBe(
      'conferenceRecords/fake-ops-002',
    )
    ...
    const beta = [...ctx.state.meetings.values()].find(
      (m) => m.title === 'Kickoff con Proveedor Beta',
    )
    expect(await reconcileMissingEvents(ctx, { meetingId: beta?.id })).toMatchObject({
      checked: 0,
      stillWaiting: 1,
    })
  ...
  it('artefactos aún no disponibles → error reintentable y FAILED tras el máximo de intentos', async () => {
    ...
    const meeting = await ctx.repos.meetings.save({
      id: ctx.ids.next(),
      googleConferenceRecordId: 'conferenceRecords/fake-ext-003',
      googleMeetingSpaceId: null,
      googleMeetingCode: 'qrs-tuvw-xyz',
      googleCalendarEventId: null,
      title: 'Interna sin artefactos',
      organizerUserId: u.lucia.id,
      organizerEmail: u.lucia.email,
      isExternalHost: false,
      startAt: now,
      endAt: now,
      durationSeconds: 0,
    ...
    await expect(
      fetchMeetingArtifacts(ctx, { meetingId: meeting.id, attempt: 1 }),
    ).rejects.toMatchObject({
      code: DomainErrorCode.GOOGLE_MEET_ARTIFACT_NOT_AVAILABLE,
      retryable: true,
    })
    ...
    expect((await ctx.repos.meetings.findById(meeting.id))?.lastErrorCode).toBe(
  ...
  it('reunión manual se ingesta, analiza heurísticamente y la retención purga el texto bruto', async () => {
    const { meetingId, enqueuedAnalysis } = await createManualMeeting(ctx, principalOf(u.mariana), {
      ...
      transcriptText:
        'Mariana Solís: Yo voy a preparar el reporte de avance del proyecto de bodega para el jueves.\nLucía Ferrer: Acordamos mover el corte de nómina al día quince.',
      confidentialityLevel: 'NORMAL',
    })
