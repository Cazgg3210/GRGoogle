...
export async function sendWeeklyDigest(
  ...
  if (principal && !hasPermission(principal, Permission.DIGEST_SEND))
    throw DomainError.forbidden('No tienes permiso para enviar el resumen semanal')
  const settings = await ctx.getSettings()
  if (!settings.featureFlags.WEEKLY_DIGEST_ENABLED)
    throw DomainError.featureDisabled('WEEKLY_DIGEST_ENABLED')
  ...
  if (recipients.length === 0)
    throw new DomainError(
      ...
      'El resumen semanal no tiene destinatarios configurados',
    )
  ...
  const saved = await ctx.uow.run(async (repos) => {
    const s = await repos.digests.save({
      ...digest,
      sentAt: res.skipped ? (digest.sentAt ?? now) : now,
      recipientEmails: recipients,
    })
    await audit(repos, ctx, {
      actorType: principal ? 'USER' : 'SYSTEM',
      actorUserId: principal?.id ?? null,
      action: res.skipped ? 'digest.send_skipped' : 'digest.sent',
      entity: 'WeeklyDigest',
      entityId: digest.id,
      after: { recipients, messageId: res.messageId },
    })
    return s
  })
  if (!res.skipped)
    await ctx.events.publish({
      type: 'WeeklyDigestSent',
      digestId: digest.id,
      recipients,
      occurredAt: now,
    })
...
): Promise<{ enabled: boolean; cron: string; timezone: string; nextRunAt: Date | null }> {
  const config = await ctx.repos.digests.getConfig()
  const cron = digestCronOf(config)
  const nextRunAt = config.enabled
    ? nextDigestRunAt(
        { dayOfWeek: config.dayOfWeek, localTime: config.localTime, timezone: config.timezone },
        ctx.clock.now(),
      )
    : null
  if (options.register ?? true) {
    if (config.enabled)
      await ctx.queue.schedule(
        JobNames.GENERATE_WEEKLY_DIGEST,
        cron,
...
export async function listDigests(
  ...
  if (
    !hasPermission(principal, Permission.REPORT_GLOBAL) &&
    !hasPermission(principal, Permission.REPORT_AREA)
  )
    throw DomainError.forbidden('No tienes permiso para ver reportes')
  const settings = await ctx.getSettings()
  return (await ctx.repos.digests.list(limit)).map((d) =>
    toDigestDto(d, settings.companyTimezone, false),
  )
}
...
export async function getDigest(
  ...
  if (!hasPermission(principal, Permission.REPORT_GLOBAL))
    throw DomainError.forbidden('No tienes permiso para ver reportes')
  const settings = await ctx.getSettings()
...
export async function getDigestConfig(
  ctx: AppContext,
  principal: Principal,
): Promise<WeeklyDigestConfigDto> {
  if (
    !hasPermission(principal, Permission.CONFIG_MANAGE) &&
    !hasPermission(principal, Permission.DIGEST_GENERATE)
  )
    throw DomainError.forbidden('No tienes permiso para ver la configuración del resumen')
...
export async function updateDigestConfig(
  ctx: AppContext,
  principal: Principal,
  patch: Partial<Omit<WeeklyDigestConfigDto, 'nextRunAt'>>,
): Promise<WeeklyDigestConfigDto> {
  if (!hasPermission(principal, Permission.CONFIG_MANAGE))
    throw DomainError.forbidden('No tienes permiso para configurar el resumen semanal')
  ...
  const next: WeeklyDigestConfig = {
    ...
    includeAreaIds:
      patch.includeAreaIds === undefined ? current.includeAreaIds : patch.includeAreaIds,
