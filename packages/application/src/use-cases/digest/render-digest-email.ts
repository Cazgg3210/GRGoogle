...
function itemLines(items: DigestItem[], extra: (i: DigestItem) => string = () => ''): string {
  return items.length === 0
    ? '  (sin elementos)'
    : items.map((i) => itemLine(i, extra(i))).join('\n')
}
...
export function renderDigestEmail(p: WeeklyDigestPayload): RenderedDigestEmail {
  ...
  const kpi = (label: string, n: number, cls = ''): string =>
    `<div style="${styles.kpi}"><span style="${styles.kpiN}${cls}">${n}</span><span style="${styles.muted}">${label}</span></div>`
  ...
  const groups = (gs: WeeklyDigestPayload['newCommitments']['byOwner'], title: string): string =>
    gs.length === 0
      ? ''
      : `<p><strong>${title}</strong></p>${gs.map((g) => `<p style="margin:6px 0 2px 0">${escapeHtml(g.label)} (${g.items.length})</p>${itemTable(g.items)}`).join('')}`
...
<div style="${styles.card}"><h2 style="${styles.h2}">C. Backlog acumulado (${p.backlog.length})</h2>${itemTable(
    p.backlog,
    ['Días abierta', 'Última mención', 'Último avance'],
    (i) => {
      const b = i as WeeklyDigestPayload['backlog'][number]
      return [String(b.daysOpen), b.lastMentionedAt ?? '-', b.lastProgressAt ?? '-']
    },
  )}</div>
...
<div style="${styles.card}"><h2 style="${styles.h2}">F. Bandeja de aprobación (${p.approvalInbox.length})</h2><p style="${styles.muted}">Tareas con cierre propuesto; requieren aprobación humana. Este correo no cambia estados.</p>${itemTable(
    p.approvalInbox,
    ['Propuesto por', 'Motivo'],
    (i) => {
      const a = i as WeeklyDigestPayload['approvalInbox'][number]
      return [escapeHtml(a.proposedBy), escapeHtml(a.reason)]
    },
  )}</div>
...
<div style="${styles.muted};text-align:center">SMLXL Meeting Intelligence · generado automáticamente</div>
  ...
  const text = [
    ...
    itemLines(
      p.backlog,
      (i) => `${(i as WeeklyDigestPayload['backlog'][number]).daysOpen} días abierta`,
    ),
    '',
    'D. RIESGOS',
    ...
    itemLines(
      p.risks.overdue,
      (i) => `${(i as WeeklyDigestPayload['risks']['overdue'][number]).daysOverdue} días vencida`,
    ),
    ...
    ...(p.risks.captureIssues.length === 0
      ? ['  (sin incidencias)']
      : p.risks.captureIssues.map(
          (c) => `- ${c.title} (${c.startAt.slice(0, 10)}): ${c.issue} | ${c.url}`,
        )),
    '',
    ...
    ...(p.changes.length === 0
      ...
      : p.changes.map(
          (c) =>
            `- ${c.key} | ${c.title} | ${c.type} | ${c.detail} | ${c.at.slice(0, 10)} | ${c.url}`,
        )),
    '',
    `F. BANDEJA DE APROBACIÓN (${p.approvalInbox.length})`,
    itemLines(
      p.approvalInbox,
      (i) => `propuesto por ${(i as WeeklyDigestPayload['approvalInbox'][number]).proposedBy}`,
    ),
    '',
    'G. PRÓXIMA SEMANA',
