...
export async function buildWeeklyDigestPayload(
  ...
  const inWeek = (d: Date | null | undefined): boolean =>
    d !== null && d !== undefined && d >= weekStart && d <= weekEnd
  ...
  const all = (await ctx.repos.actionItems.listAll({})).filter((i) => {
    if (
      config.includeAreaIds &&
      config.includeAreaIds.length > 0 &&
      i.areaId &&
      !config.includeAreaIds.includes(i.areaId)
    )
      return false
  ...
  const open = all.filter(
    (i) => OPEN_ACTION_ITEM_STATUSES.includes(i.status) && i.status !== ActionItemStatus.PROPOSED,
  )
  ...
  const overdueItems = open.filter((i) =>
  ...
  const meetingsPage = await ctx.repos.meetings.list(
  ...
  const withoutArtifacts = meetings.filter(
    (m) =>
      m.transcriptStatus === ArtifactStatus.UNAVAILABLE ||
      m.transcriptStatus === ArtifactStatus.UNAVAILABLE_EXTERNAL_HOST ||
      m.transcriptStatus === ArtifactStatus.CAPABILITY_BLOCKED,
  )
  ...
  for (const i of open.filter((x) => x.createdAt < weekStart)) {
    ...
    const lastProgress = [...history]
      .reverse()
      .find(
        (h) =>
          h.toStatus === ActionItemStatus.IN_PROGRESS ||
          h.toStatus === ActionItemStatus.COMPLETION_PROPOSED,
      )
    backlog.push({
      ...toDigestItem(i, lk, appUrl),
      daysOpen: daysOpen(i.createdAt, now, null),
      lastMentionedAt: i.lastMentionedAt ? toLocalDateString(i.lastMentionedAt, tz) : null,
      lastProgressAt: lastProgress ? toLocalDateString(lastProgress.changedAt, tz) : null,
    })
  }
  ...
  for (const i of all) {
    ...
    for (const h of history.filter((x) => inWeek(x.changedAt))) {
      if (h.toStatus === ActionItemStatus.COMPLETION_PROPOSED)
        changes.push({
          actionItemId: i.id,
          key: i.externalKey,
          title: i.title,
          type: 'POSSIBLE_COMPLETION',
          detail: h.reason ?? 'Cierre propuesto',
          at: h.changedAt.toISOString(),
          url: `${appUrl}/pendientes/${i.id}`,
        })
      if (
        h.fromStatus === ActionItemStatus.COMPLETED &&
        h.toStatus === ActionItemStatus.IN_PROGRESS
      )
        changes.push({
          actionItemId: i.id,
          ...
          title: i.title,
          type: 'REOPENED',
          detail: h.reason ?? 'Tarea reabierta',
          at: h.changedAt.toISOString(),
          url: `${appUrl}/pendientes/${i.id}`,
        })
    }
    ...
    for (const l of links.filter(
      (x) => inWeek(x.createdAt) && x.relationType === RelationType.UPDATED && x.detectedDueDate,
    )) {
      changes.push({
        actionItemId: i.id,
        key: i.externalKey,
        title: i.title,
        type: 'DUE_DATE',
        detail: `Fecha detectada en reunión: ${toLocalDateString(l.detectedDueDate as Date, tz)}`,
        at: l.createdAt.toISOString(),
        url: `${appUrl}/pendientes/${i.id}`,
      })
    }
  }
  const auditPage = await ctx.repos.audit.list(
    { entity: 'ActionItem', from: weekStart, to: weekEnd },
    { page: 1, pageSize: 500 },
  )
  for (const e of auditPage.items) {
    ...
    const base = {
      ...
      url: `${appUrl}/pendientes/${item.id}`,
    }
    if (String(before['ownerUserId'] ?? '') !== String(after['ownerUserId'] ?? ''))
      changes.push({
        ...base,
        type: 'OWNER',
        detail: `Responsable: ${lk.users.get(String(after['ownerUserId']))?.displayName ?? 'sin asignar'}`,
      })
    if (String(before['dueDate'] ?? '') !== String(after['dueDate'] ?? ''))
      changes.push({ ...base, type: 'DUE_DATE', detail: `Fecha compromiso modificada` })
    if (String(before['priority'] ?? '') !== String(after['priority'] ?? ''))
      changes.push({ ...base, type: 'PRIORITY', detail: `Prioridad: ${String(after['priority'])}` })
  }
  for (const m of meetings) {
    for (const r of await ctx.repos.aiReview.listByMeeting(m.id)) {
      ...
      const candidate = r.candidateActionItemId
        ? all.find((i) => i.id === r.candidateActionItemId)
        : undefined
      const extracted = (r.extracted ?? {}) as { title?: string }
      changes.push({
        actionItemId: candidate?.id ?? r.id,
        key: candidate?.externalKey ?? 'IA',
        title: extracted.title ?? candidate?.title ?? 'Tarea detectada',
        type: 'POSSIBLE_DUPLICATE',
        detail: `Posible duplicado detectado en "${m.title}"`,
        at: r.createdAt.toISOString(),
        url: candidate ? `${appUrl}/pendientes/${candidate.id}` : `${appUrl}/revision-ia`,
      })
    }
  }
    ...
    approvalInbox.push({
      ...
      proposedBy: proposal
        ? proposal.proposedByType === 'AI'
          ? 'IA'
          : (lk.users.get(proposal.proposedByUserId ?? '')?.displayName ?? 'Usuario')
        : 'Desconocido',
  ...
  const dueSoon = open.filter(
    (i) => i.dueDate && endOfDay(i.dueDate, tz) > weekEnd && i.dueDate <= nextWeekEnd,
  )
  ...
  const payload: WeeklyDigestPayload = {
    ...
    summary: {
      ...
      meetingsProcessed: meetings.filter((m) => processedStatuses.includes(m.processingStatus))
        .length,
    ...
    backlog,
    risks: {
      overdue: overdueItems.map((i) => ({
        ...toDigestItem(i, lk, appUrl),
        daysOverdue: Math.max(
          0,
          Math.floor((now.getTime() - endOfDay(i.dueDate as Date, tz).getTime()) / 86_400_000) + 1,
        ),
      })),
      ...
      captureIssues: [...withoutArtifacts, ...withError].map((m) => ({
        ...
        issue:
          m.processingStatus === MeetingProcessingStatus.FAILED
            ? `Error de procesamiento (${m.lastErrorCode ?? 'desconocido'})`
            : m.transcriptStatus === ArtifactStatus.UNAVAILABLE_EXTERNAL_HOST
              ? 'Organizador externo: artefactos no accesibles'
              : m.transcriptStatus === ArtifactStatus.CAPABILITY_BLOCKED
                ? 'Auto-captura bloqueada por política'
                : 'Sin transcripción ni notas',
    ...
    approvalInbox,
    nextWeek: {
      dueSoon: dueSoon.map((i) => toDigestItem(i, lk, appUrl)),
      recurring: recurring.map((i) => toDigestItem(i, lk, appUrl)),
      highPriority: highPriority.map((i) => toDigestItem(i, lk, appUrl)),
    },
    narrative: null,
  }
  ...
  if (options.withNarrative ?? true) {
    const input: WeeklyDigestInput = {
      ...
      overdueItems: payload.risks.overdue.map((i) => ({
        key: i.key,
        title: i.title,
        owner: i.owner,
      ...
      captureIssues: payload.risks.captureIssues.map((c) => ({
    ...
    try {
      ...
      if (validated.success) payload.narrative = validated.data as WeeklyDigestResult
        ...
        ctx.logger.warn(
          ...
          'Narrativa del digest inválida; se omite',
        )
    } catch (err) {
      ctx.logger.warn(
        { errorCode: err instanceof DomainError ? err.code : 'ERR' },
        'Narrativa IA del digest falló; se continúa sin narrativa',
      )
    }
  }
  return payload
...
export async function generateWeeklyDigest(
  ...
  if (principal && !hasPermission(principal, Permission.DIGEST_GENERATE))
    throw DomainError.forbidden('No tienes permiso para generar el resumen semanal')
  const settings = await ctx.getSettings()
  const tz = settings.companyTimezone
  ...
  const payload = await buildWeeklyDigestPayload(ctx, weekOf, {
  ...
  const digest = await ctx.uow.run(async (repos) => {
    ...
    await audit(repos, ctx, {
      ...
      entity: 'WeeklyDigest',
      entityId: saved.id,
      after: { weekLabel: week.label, version: saved.version, summary: payload.summary },
    })
    return saved
  })
