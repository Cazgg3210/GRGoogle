...
import { createActionItem, proposeCompletion, approveCompletion, updateActionItem } from '../action-items/commands.js'
...
import { digestCronOf, getDigest, scheduleWeeklyDigest, sendWeeklyDigest, updateDigestConfig } from './send-weekly-digest.js'
...
describe('resumen semanal', () => {
  it('calcula las secciones A–G, versiona, renderiza y envía con idempotencia', async () => {
    ...
    const old = await createActionItem(ctx, director, { title: 'Actualizar manual de procesos', dueDate: '2026-08-31', priority: 'HIGH' })
    const recurring = await createActionItem(ctx, director, { title: 'Seguimiento diario a proveedor', ownerUserId: u.lucia.id, type: 'RECURRING', recurrence: { frequency: 'DAILY' } })
    ...
    const done = await createActionItem(ctx, director, { title: 'Enviar factura de agosto', ownerUserId: u.mariana.id, dueDate: '2026-09-04' })
    ...
    const blocked = await createActionItem(ctx, director, { title: 'Liberar rutas', ownerUserId: u.lucia.id, dueDate: '2026-09-09' })
    await updateActionItem(ctx, principalOf(u.lucia), blocked.id, { status: ActionItemStatus.BLOCKED, blocker: 'esperando póliza' })
    ...
    expect(p.summary).toMatchObject({ meetingsDetected: 4, meetingsProcessed: 1, meetingsWithoutArtifacts: 1, meetingsWithError: 0, newActionItems: 5, pendingProposals: 0, approvedCompletions: 1, blocked: 1 })
    ...
    expect(p.backlog.map((b) => b.key)).toEqual(expect.arrayContaining([old.externalKey, recurring.externalKey]))
    ...
    expect(p.changes.map((c) => c.type)).toEqual(expect.arrayContaining(['POSSIBLE_COMPLETION', 'DUE_DATE']))
    expect(p.approvalInbox).toEqual([])
    expect(p.nextWeek.dueSoon.map((i) => i.key)).toEqual(expect.arrayContaining([old.externalKey, blocked.externalKey]))
    ...
    expect(p.risks.captureIssues.map((c) => c.issue)).toEqual(['Auto-captura bloqueada por política'])
    for (const i of p.newCommitments.items) expect(i.url).toBe(`${ctx.env.APP_URL}/pendientes/${i.id}`)

    const email = renderDigestEmail(p)
    expect(email.subject).toContain('2026-W36')
    for (const section of ['A. Resumen ejecutivo', 'B. Nuevos compromisos', 'C. Backlog acumulado', 'D. Riesgos', 'E. Cambios detectados', 'F. Bandeja de aprobación', 'G. Próxima semana']) expect(email.html).toContain(section)
    ...
    expect(ctx.events.events.map((e) => e.type)).toEqual(expect.arrayContaining(['WeeklyDigestGenerated', 'WeeklyDigestSent']))
  })

  it('bandeja de aprobación incluye COMPLETION_PROPOSED con enlace y respeta flags', async () => {
    const director = principalOf(u.andres)
    const item = await createActionItem(ctx, director, { title: 'Cerrar contrato', ownerUserId: u.lucia.id })
    ...
    const p = digest.payload as WeeklyDigestPayload
    expect(p.approvalInbox[0]).toMatchObject({ key: item.externalKey, proposedBy: 'Lucía Ferrer', url: `${ctx.env.APP_URL}/pendientes/${item.id}` })
    expect(p.narrative).toBeNull()
    await updateDigestConfig(ctx, principalOf(u.admin), { sendEmail: false })
    await expect(sendWeeklyDigest(ctx, director, digest.id)).rejects.toMatchObject({ code: DomainErrorCode.FEATURE_DISABLED })
    await expect(sendWeeklyDigest(ctx, principalOf(u.mariana), digest.id)).rejects.toMatchObject({ code: DomainErrorCode.FORBIDDEN })
  })
  ...
  it('programación: cron y próxima ejecución desde la configuración', async () => {
    ...
    expect(ctx.queue.schedules[0]).toMatchObject({ name: 'generate-weekly-digest', cron: '0 18 * * 5', timezone: 'America/Mexico_City' })
  })
})
