...
} from './reconcile-action-items.js'
...
function extracted(partial: Partial<ExtractedActionItem> & { title: string }): ExtractedActionItem {
  return {
    ...
    priority: null,
    statusHint: 'NEW',
    evidence: [{ text: 'evidencia', speaker: 'Andrés Escandón' }],
    confidence: 0.95,
    ...partial,
  }
}
...
async function seedItem(partial: Partial<ActionItem> & { title: string }): Promise<ActionItem> {
  ...
  await ctx.repos.actionItems.addStatusHistory({
    id: ctx.ids.next(),
...
async function reconcile(items: ExtractedActionItem[]) {
  ...
  const rc: ReconcileContext = {
    meeting,
    run,
    participants: await ctx.repos.meetings.listParticipants(meeting.id),
    settings,
...
beforeEach(async () => {
  ...
  meeting = await ctx.repos.meetings.save({
    id: ctx.ids.next(),
    googleConferenceRecordId: 'conferenceRecords/x',
    googleMeetingSpaceId: null,
    googleMeetingCode: 'abc-defg-hij',
    googleCalendarEventId: null,
    title: 'Seguimiento contrato Cliente Alfa',
    organizerUserId: users.andres.id,
    organizerEmail: users.andres.email,
    ...
    processingStatus: MeetingProcessingStatus.ANALYZING,
    transcriptStatus: 'INGESTED',
    smartNotesStatus: 'INGESTED',
    ...
    lastErrorCode: null,
    lastErrorAt: null,
    areaId: users.areas.direccion.id,
    projectId: null,
    createdAt: now,
    updatedAt: now,
  })
  await ctx.repos.meetings.replaceParticipants(meeting.id, [
    {
      ...
      internalUserId: users.andres.id,
      googleParticipantId: 'p1',
      displayName: 'Andrés Escandón',
      email: users.andres.email,
      participantType: 'SIGNED_IN_USER',
      isInternal: true,
      joinedAt: null,
      leftAt: null,
    ...
    },
    {
      id: ctx.ids.next(),
      meetingId: meeting.id,
      internalUserId: users.lucia.id,
      googleParticipantId: 'p2',
      displayName: 'Lucía Ferrer',
      email: users.lucia.email,
      participantType: 'SIGNED_IN_USER',
      isInternal: true,
      joinedAt: null,
      leftAt: null,
  ...
  run = await ctx.repos.processingRuns.save({
    id: ctx.ids.next(),
    meetingId: meeting.id,
    kind: 'ANALYZE_MEETING',
    provider: 'fake',
    model: 'fake',
    promptVersion: 'v1',
    schemaVersion: '1.0.0',
...
describe('scoreCandidate', () => {
  it('pondera título, responsable, proyecto y clave referenciada', async () => {
    ...
    const s = scoreCandidate(
      { title: 'Revisar anexo de penalizaciones', relatedOpenActionKey: item.externalKey },
      item,
      { userId: users.lucia.id, externalAssigneeId: null },
      'p1',
    )
    ...
    const weak = scoreCandidate(
      ...
      item,
      { userId: null, externalAssigneeId: null },
      null,
    )
...
describe('reconciliación', () => {
  it('CREATE_NEW: banda AUTO_ACCEPT crea PENDING con responsable resuelto por nombre y fecha', async () => {
    const { plan, summary } = await reconcile([
      extracted({
        title: 'Enviar propuesta comercial a Cliente Beta',
        owner: { name: 'Lucia Ferrer', evidence: 'yo la envío' },
        dueDate: '2026-09-08',
        dueDateTextOriginal: 'el martes',
        confidence: 0.95,
      }),
    ])
    ...
    expect(
      (await ctx.repos.actionItems.listLinks(item?.id ?? '')).map((l) => l.relationType),
    ).toEqual(['CREATED'])
    expect(
      ctx.state.audit.some((a) => a.action === 'action_item.created' && a.actorType === 'AI'),
    ).toBe(true)
  ...
  it('banda PROPOSAL crea PROPOSED + revisión LOW_CONFIDENCE; banda REVIEW sólo revisión', async () => {
    const { summary } = await reconcile([
      extracted({
        title: 'Preparar plan de onboarding',
        owner: { name: 'Lucía Ferrer', evidence: 'x' },
        confidence: 0.75,
      }),
    ...
    expect(reviews.find((r) => r.proposedActionItemId === items[0]?.id)?.reasons).toEqual([
      'LOW_CONFIDENCE',
    ])
  ...
  it('responsable externo crea ExternalAssignee; área como responsable y fecha ambigua generan razones', async () => {
    const { summary } = await reconcile([
      extracted({
        title: 'Enviar carta de intención firmada',
        owner: {
      ...
      extracted({
        title: 'Revisar anexo de penalizaciones',
        owner: { name: 'Jurídico', evidence: 'x' },
        dueDateTextOriginal: 'cuando se pueda',
        confidence: 0.95,
      }),
    ])
    ...
    const review = (await ctx.repos.aiReview.listPending()).find(
      (r) => r.proposedActionItemId === anexo?.id,
    )
    expect(review?.reasons).toEqual(
      expect.arrayContaining(['AMBIGUOUS_OWNER', 'AMBIGUOUS_DUE_DATE']),
    )
  })
  ...
  it('LINK_EXISTING cuando coincide fuertemente y sólo se menciona', async () => {
    ...
    const { plan, summary } = await reconcile([
      extracted({
        title: 'Revisar anexo de penalizaciones del contrato',
        owner: { name: 'Lucía Ferrer', evidence: 'x' },
      }),
    ])
  ...
  it('UPDATE_EXISTING aplica nueva fecha con confianza alta; con confianza media crea CONFLICT_WITH_EXISTING', async () => {
    ...
    const { plan, summary } = await reconcile([
      extracted({
        title: 'Enviar reporte mensual de ventas',
        owner: { name: 'Lucía Ferrer', evidence: 'x' },
        statusHint: 'UPDATE',
        dueDate: '2026-09-15',
        confidence: 0.95,
      ...
      extracted({
        title: 'Actualizar inventario de bodega norte',
        owner: { name: 'Lucía Ferrer', evidence: 'x' },
        statusHint: 'UPDATE',
        dueDate: '2026-09-20',
        confidence: 0.8,
    ...
    expect((await ctx.repos.actionItems.findById(a.id))?.dueDate?.toISOString()).toBe(
    ...
    expect(review).toMatchObject({
      candidateActionItemId: b.id,
      reasons: ['CONFLICT_WITH_EXISTING'],
    })
  ...
  it('BLOCKED nunca cambia el estado: vincula BLOCKED y crea revisión', async () => {
    ...
    await reconcile([
      extracted({
        title: 'Liberar rutas nuevas de transporte',
        owner: { name: 'Lucía Ferrer', evidence: 'x' },
        statusHint: 'BLOCKED',
        description: 'esperando póliza',
        confidence: 0.95,
      }),
    ])
    ...
    expect((await ctx.repos.actionItems.listLinks(item.id)).map((l) => l.relationType)).toEqual([
      'BLOCKED',
    ])
  ...
  it('MARK_DONE_CANDIDATE propone cierre (COMPLETION_PROPOSED) y nunca COMPLETED', async () => {
    const item = await seedItem({
      title: 'Enviar presupuesto de licencias',
    ...
    expect(proposal).toMatchObject({
      proposedByType: 'AI',
      proposedFromMeetingId: meeting.id,
      confidence: 0.86,
    })
    expect((await ctx.repos.actionItems.listStatusHistory(item.id)).at(-1)).toMatchObject({
      fromStatus: 'IN_PROGRESS',
      toStatus: 'COMPLETION_PROPOSED',
      changedBySystem: true,
    })
    ...
    expect(
      [...ctx.state.actionItems.values()].every((i) => i.status !== ActionItemStatus.COMPLETED),
    ).toBe(true)
  })
  ...
  it('MARK_DONE con flag AI_COMPLETION_PROPOSALS_ENABLED=false sólo crea revisión', async () => {
    ...
    await ctx.repos.settings.save(
      { ...s, featureFlags: { ...s.featureFlags, AI_COMPLETION_PROPOSALS_ENABLED: false } },
      null,
    )
  ...
  it('REOPEN_CANDIDATE: candidato completado nunca se reabre automáticamente', async () => {
    ...
    const { plan } = await reconcile([
      extracted({
        title: 'Configurar servidor de respaldo',
        owner: { name: 'Lucía Ferrer', evidence: 'x' },
        relatedOpenActionKey: item.externalKey,
        statusHint: 'UPDATE',
      }),
    ])
  ...
  it('zona gris usa el juez LLM: REQUIRES_HUMAN_REVIEW → POSSIBLE_DUPLICATE sin crear tarea', async () => {
    const item = await seedItem({
      title: 'Preparar reporte de avance del proyecto bodega',
      ownerUserId: users.mariana.id,
    })
    const { plan, summary } = await reconcile([
      extracted({
        title: 'Preparar reporte de avance de bodega',
        owner: { name: 'Mariana Solís', evidence: 'x' },
        confidence: 0.95,
      }),
    ])
