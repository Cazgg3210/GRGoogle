...
async function candidatesMeetings(ctx: AppContext, meetingId?: string): Promise<Meeting[]> {
  ...
  const waiting = await ctx.repos.meetings.listByStatus(
...
export async function reconcileMissingEvents(
...
): Promise<ReconcileMissingEventsResult> {
  const settings = await ctx.getSettings()
  const result: ReconcileMissingEventsResult = {
    checked: 0,
    linked: 0,
    unavailableExternal: 0,
    stillWaiting: 0,
    failed: 0,
  }
  ...
  for (const meeting of await candidatesMeetings(ctx, input.meetingId)) {
    ...
    const internal =
      (meeting.organizerEmail && isInternalEmail(meeting.organizerEmail, settings.companyDomain)
        ? meeting.organizerEmail
        : null) ??
      participants.find(
        (p) => p.isInternal && p.email && isInternalEmail(p.email, settings.companyDomain),
      )?.email ??
      null
    try {
      const records = internal
        ? await ctx.meet.listConferenceRecordsByMeetingCode(meeting.googleMeetingCode, internal)
        : []
      const match = records
        .filter(
          (r) => Math.abs(r.startTime.getTime() - meeting.startAt.getTime()) < 6 * 60 * 60 * 1000,
        )
        .sort(
          (a, b) =>
            Math.abs(a.startTime.getTime() - meeting.startAt.getTime()) -
            Math.abs(b.startTime.getTime() - meeting.startAt.getTime()),
        )[0]
      if (match) {
        await ctx.uow.run(async (repos) => {
          ...
          const saved = await repos.meetings.save({
            ...
            durationSeconds: endAt
              ? Math.round((endAt.getTime() - match.startTime.getTime()) / 1000)
              : m.durationSeconds,
            transcriptStatus:
              m.transcriptStatus === ArtifactStatus.NOT_REQUESTED ||
              m.transcriptStatus === ArtifactStatus.CAPABILITY_BLOCKED
                ? ArtifactStatus.PENDING
                : m.transcriptStatus,
            smartNotesStatus:
              m.smartNotesStatus === ArtifactStatus.NOT_REQUESTED ||
              m.smartNotesStatus === ArtifactStatus.CAPABILITY_BLOCKED
                ? ArtifactStatus.PENDING
                : m.smartNotesStatus,
            updatedAt: now,
          })
          if (saved.processingStatus === MeetingProcessingStatus.DISCOVERED)
            await setProcessingStatus(repos, saved, MeetingProcessingStatus.WAITING_FOR_ARTIFACTS)
          await audit(repos, ctx, {
            actorType: 'SYSTEM',
            action: 'meeting.conference_record_reconciled',
            entity: 'Meeting',
            entityId: m.id,
            after: { conferenceRecord: match.name, asUser: internal },
          })
        })
        await enqueueJob(
          ctx,
          JobNames.FETCH_MEETING_ARTIFACTS,
          { meetingId: meeting.id },
          { singletonKey: `fetch:${meeting.id}`, startAfterSeconds: 60 },
        )
        result.linked += 1
        continue
      }
      if (meeting.isExternalHost || !internal) {
        await ctx.uow.run(async (repos) => {
          ...
          let cur = m
          if (cur.processingStatus === MeetingProcessingStatus.DISCOVERED)
            cur = await setProcessingStatus(
              repos,
              cur,
              MeetingProcessingStatus.WAITING_FOR_ARTIFACTS,
            )
      ...
      if (now.getTime() - end.getTime() > FAIL_AFTER_MS) {
        await ctx.uow.run(async (repos) => {
          ...
          await setProcessingStatus(repos, cur, MeetingProcessingStatus.FAILED, {
            status: MeetingStatus.ENDED,
        ...
        await ctx.uow.run(async (repos) => {
          ...
          if (m.processingStatus === MeetingProcessingStatus.DISCOVERED)
            await setProcessingStatus(repos, m, MeetingProcessingStatus.WAITING_FOR_ARTIFACTS)
        })
        await enqueueJob(
          ctx,
          JobNames.RECONCILE_MISSING_EVENTS,
          { meetingId: meeting.id },
          { singletonKey: `reconcile:${meeting.id}`, startAfterSeconds: 60 * 60 },
        )
        result.stillWaiting += 1
      }
    } catch (err) {
      ...
      await ctx.repos.meetings.updateProcessing(meeting.id, {
        lastErrorCode: code,
        lastErrorAt: now,
      })
      result.failed += 1
