...
export async function buildAnalyzeInput(
  ...
  const main =
    byNewest.find((t) => t.sourceType === TranscriptSourceType.MEET_TRANSCRIPT) ??
    byNewest.find((t) => t.sourceType === TranscriptSourceType.MANUAL) ??
    null
  ...
  if (segments.length === 0 && !notes) {
    throw new DomainError(
      DomainErrorCode.TRANSCRIPT_EMPTY,
      'La reunión no tiene transcripción ni notas para analizar',
      { details: { meetingId: meeting.id } },
    )
  }
  const participants = await repos.meetings.listParticipants(meeting.id)
  const participantUserIds = new Set(
    participants.map((p) => p.internalUserId).filter((id): id is string => id !== null),
  )
  const users = await repos.users.list()
  const areaIds = new Set(
    users
      .filter((u) => participantUserIds.has(u.id))
      .map((u) => u.areaId)
  ...
  const related = open
    .filter(
      (a) =>
        (a.ownerUserId && participantUserIds.has(a.ownerUserId)) ||
        (a.areaId && areaIds.has(a.areaId)) ||
        (meeting.projectId && a.projectId === meeting.projectId),
    )
    .sort(
      (a, b) =>
        (b.lastMentionedAt?.getTime() ?? b.updatedAt.getTime()) -
        (a.lastMentionedAt?.getTime() ?? a.updatedAt.getTime()),
    )
    .slice(0, 40)
  ...
  const openActions: OpenActionContext[] = related.map((a) => ({
    ...
    ownerName: a.ownerUserId
      ? (users.find((u) => u.id === a.ownerUserId)?.displayName ?? null)
      : a.ownerTextOriginal,
    status: a.status,
    dueDate: a.dueDate ? toLocalDateString(a.dueDate, settings.companyTimezone) : null,
    projectName: a.projectId
      ? (projects.find((p) => p.id === a.projectId)?.canonicalName ?? null)
      : null,
  }))
  return {
    meeting: {
    ...
    segments: segments.map((s) => ({
      ...
      text: s.text,
      startTime: s.startAt?.toISOString() ?? null,
      endTime: s.endAt?.toISOString() ?? null,
    })),
...
export async function analyzeMeeting(
  ctx: AppContext,
  input: { meetingId: string; kind?: ProcessingRun['kind']; correlationId?: string },
  ...
  if (meeting.excludedFromAi) {
    await ctx.uow.run(async (repos) => {
      ...
      if (m.processingStatus === MeetingProcessingStatus.INGESTED)
  ...
  try {
    analyzeInput = await ctx.uow.run(async (repos) => {
      ...
      await setProcessingStatus(repos, m, MeetingProcessingStatus.ANALYZING, {
    ...
    await ctx.repos.processingRuns.save({
      ...run,
      success: false,
      errorCode: code,
  ...
  try {
    ...
    if (!validated.success) {
      throw new DomainError(
        ...
        'La salida del modelo no cumple el schema',
        {
          details: {
    ...
    await ctx.uow.run(async (repos) => {
      ...
      await audit(repos, ctx, {
        ...
        entity: 'Meeting',
        entityId: meeting.id,
        after: { runId: run.id, errorCode: code },
        correlationId,
      })
    })
  ...
  const result = await ctx.uow.run(async (repos) => {
    ...
    const summary: MeetingSummary = {
      ...
      risks: [
        ...
        ...analysis.summary.attentionPoints.map((a) => `Atención: ${a}`),
      ],
    ...
    current = await setProcessingStatus(repos, current, MeetingProcessingStatus.ANALYZED, {
      aiAnalysisStatus: 'SUCCEEDED',
      lastErrorCode: null,
      lastErrorAt: null,
    })
    current = await setProcessingStatus(
      repos,
      current,
      reviewRequired ? MeetingProcessingStatus.REVIEW_REQUIRED : MeetingProcessingStatus.COMPLETED,
    )
    await audit(repos, ctx, {
      actorType: 'AI',
