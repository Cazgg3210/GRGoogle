...
import { UserRole, canAccessActionItem, canAccessMeeting, normalizeText, type Principal } from '@smlxl/domain'
 ...
 */
export async function searchKnowledge(ctx: AppContext, principal: Principal, q: string, limit = 20): Promise<SearchResultDto> {
  ...
  const items = (await ctx.repos.actionItems.searchFullText(q, { openOnly: false, limit })).filter((i) => canAccessActionItem(principal, i))
  const actionItems = items.map((i) => {
    ...
    return { id: i.id, externalKey: i.externalKey, title: i.title, status: i.status, ownerName: i.ownerUserId ? (lk.users.get(i.ownerUserId)?.displayName ?? null) : i.ownerTextOriginal, snippet: snippet(`${i.title}. ${i.description ?? ''}`, q) }
  })
  ...
  for (const m of meetingsPage.items) {
    ...
    meetings.push({ id: m.id, title: m.title, startAt: m.startAt.toISOString(), snippet: snippet(summary?.executiveSummary.join(' ') ?? m.title, q) })
  }
  ...
  for (const m of candidates) {
    ...
    for (const d of await ctx.repos.decisions.listByMeeting(m.id)) {
      ...
      if (tokens.length > 0 && tokens.some((t) => nd.includes(t))) {
        sourceMeetingIds.add(m.id)
        decisions.push({ id: d.id, meetingId: m.id, meetingTitle: m.title, description: d.description })
