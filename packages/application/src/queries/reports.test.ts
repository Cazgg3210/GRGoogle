...
import { approveCompletion, createActionItem, proposeCompletion, updateActionItem } from '../use-cases/action-items/commands.js'
...
import { PENDIENTES_COLUMNS, REUNIONES_COLUMNS, syncTasksToGoogleSheets } from '../use-cases/sheets/sync-tasks-to-google-sheets.js'
...
beforeEach(async () => {
  ctx = createTestContext({ now: '2026-09-03T18:00:00Z', env: { GOOGLE_SHEETS_SPREADSHEET_ID: 'sheet-demo' } })
  u = await seedDemoUsers(ctx)
})

async function seedScenario() {
  const director = principalOf(u.andres)
  const overdue = await createActionItem(ctx, director, { title: 'Entregar reporte fiscal', ownerUserId: u.mariana.id, dueDate: '2026-08-30', priority: 'URGENT', areaId: u.areas.operaciones.id })
  const soon = await createActionItem(ctx, director, { title: 'Enviar cotización', ownerUserId: u.mariana.id, dueDate: '2026-09-05', areaId: u.areas.operaciones.id })
  const noDate = await createActionItem(ctx, director, { title: 'Definir plan de capacitación', ownerUserId: u.lucia.id, areaId: u.areas.juridico.id })
  const done = await createActionItem(ctx, director, { title: 'Firmar convenio', ownerUserId: u.lucia.id, dueDate: '2026-09-02', areaId: u.areas.juridico.id })
  ...
  await updateActionItem(ctx, principalOf(u.lucia), noDate.id, { status: ActionItemStatus.IN_PROGRESS })
...
describe('dashboard y listados', () => {
  it('calcula KPIs, por área/persona, tendencia, atención y calidad de captura respetando alcance', async () => {
    ...
    expect(dash.kpis).toMatchObject({ completedInPeriod: 1, overdue: 1, meetingsDetected: 2, meetingsProcessed: 1 })
    ...
    expect(dash.captureQuality).toMatchObject({ detected: 2, withTranscript: 1, withSmartNotes: 1, noArtifact: 1, externalHostUnavailable: 0 })
  ...
  it('listActionItems implementa las vistas y ordena por atención', async () => {
    ...
    expect((await listActionItems(ctx, director, { view: 'overdue' })).items.map((i) => i.id)).toEqual([overdue.id])
    expect((await listActionItems(ctx, director, { view: 'completed' })).total).toBe(1)
    expect((await listActionItems(ctx, director, { view: 'noDueDate' })).items.some((i) => i.id === noDate.id)).toBe(true)
    expect((await listActionItems(ctx, director, { view: 'thisWeek' })).items.map((i) => i.title)).toContain('Enviar cotización')
    ...
    const team = await listActionItems(ctx, principalOf(u.lucia, { teamUserIds: [u.mariana.id], managedAreaIds: [u.areas.juridico.id] }), { view: 'team' })
    expect(team.total).toBe(5)
    const paged = await listActionItems(ctx, director, { view: 'all', sort: 'dueDate', page: 2, pageSize: 2 })
...
describe('proyección a Sheets', () => {
  it('genera Pendientes/Reuniones con UUID como clave y hace upsert idempotente', async () => {
    ...
    expect(dry.preview.pendientes.rows.find((r) => r['Actividad'] === 'Entregar reporte fiscal')).toMatchObject({ Vencido: 'Sí', Responsable: 'Mariana Solís', Área: 'Operaciones y Proyectos', 'Fecha compromiso': '2026-08-30' })
    expect(dry.preview.reuniones.rows.find((r) => r['Reunión'] === 'Seguimiento contrato Cliente Alfa')).toMatchObject({ Organizador: 'Andrés Escandón', '# Acuerdos': 2, '# Tareas nuevas': 3 })
    ...
    await ctx.repos.settings.save({ ...s, featureFlags: { ...s.featureFlags, SHEETS_SYNC_ENABLED: false } }, null)
    await expect(syncTasksToGoogleSheets(ctx, principalOf(u.andres), {})).rejects.toMatchObject({ code: DomainErrorCode.FEATURE_DISABLED })
    await expect(syncTasksToGoogleSheets(ctx, principalOf(u.mariana), { dryRun: true })).rejects.toMatchObject({ code: DomainErrorCode.FORBIDDEN })
  })
})
...
describe('recordatorios', () => {
  it('agrupa por usuario un correo al día (idempotente) y respeta preferencias/flag', async () => {
    ...
    expect(first).toMatchObject({ users: 1, emailsSent: 1, emailsSkipped: 0, overdue: 1, dueSoon: 1, disabled: false })
    ...
    await ctx.repos.users.save({ ...mariana, notificationPreferences: { ...mariana.notificationPreferences, dueSoon: false, overdue: false } })
    ctx.clock.advanceDays(1)
    expect((await sendReminders(ctx)).users).toBe(0)
    const s = await ctx.getSettings()
    await ctx.repos.settings.save({ ...s, featureFlags: { ...s.featureFlags, GMAIL_NOTIFICATIONS_ENABLED: false } }, null)
