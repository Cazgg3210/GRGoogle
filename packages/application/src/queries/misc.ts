...
export async function listAiReviewItems(
  ctx: AppContext,
  principal: Principal,
  q: ListAiReviewQuery = {},
): Promise<Page<AiReviewItemDto>> {
  if (!hasPermission(principal, Permission.AI_REVIEW_RESOLVE))
    throw DomainError.forbidden('No tienes permiso para ver la bandeja de revisión IA')
  const settings = await ctx.getSettings()
  let pending = await ctx.repos.aiReview.listPending(q.meetingId ? { meetingId: q.meetingId } : {})
  if (q.reason)
    pending = pending.filter((r) => (r.reasons as string[]).includes(q.reason as string))
  const page = paginate(pending, q.page ?? 1, q.pageSize ?? 25)
  const lk = await loadLookups(
    ctx.repos,
    settings,
    ctx.clock.now(),
    page.items.map((r) => r.meetingId),
  )
  const items: AiReviewItemDto[] = []
  for (const r of page.items) {
    const candidate = r.candidateActionItemId
      ? await ctx.repos.actionItems.findById(r.candidateActionItemId)
      : null
...
export async function getAiReviewItem(
  ctx: AppContext,
  principal: Principal,
  id: string,
): Promise<AiReviewItemDto> {
  if (!hasPermission(principal, Permission.AI_REVIEW_RESOLVE))
    throw DomainError.forbidden('No tienes permiso para ver la bandeja de revisión IA')
...
export async function listReviewItemsByMeeting(
  ctx: AppContext,
  principal: Principal,
  meetingId: string,
): Promise<AiReviewItemDto[]> {
...
export async function getGoogleStatus(
  ctx: AppContext,
  principal: Principal,
): Promise<GoogleStatusDto> {
  if (!hasPermission(principal, Permission.INTEGRATION_MANAGE))
    throw DomainError.forbidden('No tienes permiso para ver integraciones')
  const settings = await ctx.getSettings()
  ...
  return {
    ...
    subscriptions: subs.map((s) => ({
      userEmail: s.monitoredUserEmail,
      subscriptionName: s.googleSubscriptionName,
      state: s.state,
      expiresAt: s.expiresAt.toISOString(),
      lastRenewedAt: s.lastRenewedAt?.toISOString() ?? null,
      lastErrorCode: s.lastErrorCode,
    })),
    calendarCursors: cursors.map((c) => ({
      userEmail: emailOf(c.userId),
...
export async function listAuditEntries(
  ...
  if (!hasPermission(principal, Permission.AUDIT_READ))
    throw DomainError.forbidden('No tienes permiso para ver auditoría')
  const settings = await ctx.getSettings()
...
export async function listMeetingAudit(
  ctx: AppContext,
  principal: Principal,
  meetingId: string,
  ...
  const settings = await ctx.getSettings()
  const page = await ctx.repos.audit.list(
...
export async function listExternalAssignees(
  ctx: AppContext,
  activeOnly = true,
): Promise<ExternalAssigneeDto[]> {
...
export async function getNotificationCounts(
  ...
  const pendingAiReview = hasPermission(principal, Permission.AI_REVIEW_RESOLVE)
    ? await ctx.repos.aiReview.countPending()
    : 0
