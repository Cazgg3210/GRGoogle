...
> {
  ...
  for (const m of meetings) {
    ...
    if (
      principal &&
      !canAccessMeeting(principal, { ...m, participantUserIds: participantUserIds(participants) })
    ) {
      out.push(null)
      continue
    }
...
export async function listMeetings(
  ...
  principal: Principal,
  q: ListMeetingsQuery = {},
): Promise<Page<MeetingListItemDto>> {
  ...
  const items = enriched
    ...
    .map((e) => toMeetingListItemDto(e.meeting, lk, e))
...
export async function getMeetingDetail(
  ...
  const lk = await loadLookups(ctx.repos, settings, ctx.clock.now())
  return toMeetingDetailDto(meeting, lk, {
    participants,
    actionItemCount,
    pendingReviewCount: reviews.length,
    summary,
...
export async function getMeetingTranscript(
  ...
  if (!hasPermission(principal, Permission.MEETING_READ_TRANSCRIPT))
    throw DomainError.forbidden('No tienes permiso para leer transcripciones')
  const meeting = requireMeeting(await ctx.repos.meetings.findById(id), id)
