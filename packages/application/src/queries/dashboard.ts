...
function kpiRow(
  ...
  return {
    ...
    pending: open.filter(
      (i) => i.status === ActionItemStatus.PENDING || i.status === ActionItemStatus.PROPOSED,
    ).length,
    completionProposed: open.filter((i) => i.status === ActionItemStatus.COMPLETION_PROPOSED)
      .length,
    overdue: open.filter((i) => isOverdue({ dueDate: i.dueDate, status: i.status }, now, tz))
      .length,
...
export async function getDashboard(
  ...
  const from =
    (q.from ? parseLocalDate(q.from, tz) : null) ?? new Date(now.getTime() - 30 * 86_400_000)
  ...
  const scopeFilter = (i: ActionItem): boolean =>
    (global || canAccessActionItem(principal, i)) &&
    (!q.areaId || i.areaId === q.areaId) &&
    (!q.projectId || i.projectId === q.projectId)
  const open = (
    await ctx.repos.actionItems.listAll({ status: [...OPEN_ACTION_ITEM_STATUSES] })
  ).filter(scopeFilter)
  const completedInPeriod = (
    await ctx.repos.actionItems.listAll({
  ...
  const byArea = areas
    .map((a) =>
      kpiRow(
        a.id,
        a.name,
        open.filter((i) => i.areaId === a.id),
        completedInPeriod.filter((i) => i.areaId === a.id),
        now,
        tz,
      ),
    )
  ...
  const noArea = kpiRow(
    'none',
    'Sin área',
    open.filter((i) => !i.areaId),
    completedInPeriod.filter((i) => !i.areaId),
    now,
    tz,
  )
  ...
  const byPerson = [...ownerIds]
    .map((id) =>
      kpiRow(
        id,
        lk.users.get(id)?.displayName ?? id,
        open.filter((i) => i.ownerUserId === id),
        completedInPeriod.filter((i) => i.ownerUserId === id),
        now,
        tz,
      ),
    )
  ...
  const weeklyTrend = previousWeeks(now, 8, tz).map((w) => {
    const created = allItems.filter(
      (i) => i.createdAt >= w.weekStart && i.createdAt <= w.weekEnd,
    ).length
    const completed = allItems.filter(
      (i) => i.completedAt && i.completedAt >= w.weekStart && i.completedAt <= w.weekEnd,
    ).length
    const openAtEnd = allItems.filter(
      (i) =>
        i.createdAt <= w.weekEnd &&
        (!i.completedAt || i.completedAt > w.weekEnd) &&
        (!i.cancelledAt || i.cancelledAt > w.weekEnd),
    )
    const overdueAtEnd = openAtEnd.filter(
      (i) => i.dueDate && endOfDay(i.dueDate, tz).getTime() < w.weekEnd.getTime(),
    ).length
    return {
      week: w.label,
  ...
  for (const i of open) {
    ...
    scored.push(
      toActionItemDto(i, lk, {
        pendingProposalId: pendingByItem.get(i.id) ?? null,
        mentionsWithoutProgress: mentions,
      }),
    )
  }
  const needsAttention = scored
    .filter((d) => d.attentionScore > 0)
  ...
  for (const m of recent) {
    ...
    if (
      ...
      continue
    const [actionItemCount, reviews] = await Promise.all([
      ctx.repos.meetings.countActionItems(m.id),
      ctx.repos.aiReview.listPending({ meetingId: m.id }),
    ])
    recentMeetings.push(
      toMeetingListItemDto(m, lk, {
        participants,
        actionItemCount,
  ...
  return {
    ...
    kpis: {
      ...
      meetingsProcessed: meetingsInPeriod.filter((m) =>
        processedStatuses.includes(m.processingStatus),
      ).length,
