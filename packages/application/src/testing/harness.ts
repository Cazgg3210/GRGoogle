...
import {
  createFakeGoogleAdapters,
  type FakeGoogleAdapters,
...
export function createTestContext(options: TestContextOptions = {}): TestContext {
  ...
  const google = createFakeGoogleAdapters(env, {
    now: () => clock.now(),
    fixtures: options.fixtures,
    monitoredUserEmails: monitored,
  })
...
export async function seedDemoUsers(ctx: TestContext): Promise<SeededUsers> {
  ...
  const andres = await user(
    'andres.escandon@smlxl.mx',
    'Andrés Escandón',
    UserRole.DIRECTOR,
    direccion.id,
  )
  const lucia = await user(
  ...
  const mariana = await user(
    'mariana.solis@smlxl.mx',
    'Mariana Solís',
    UserRole.MEMBER,
    operaciones.id,
    lucia.id,
  ...
  await ctx.repos.settings.save(
    { ...settings, monitoredUserEmails: [andres.email, lucia.email, mariana.email] },
    null,
  )
  const known = new Set(ctx.google.directory.users.map((d) => d.email))
  ;[andres, lucia, mariana, admin].forEach((u, i) => {
    if (!known.has(u.email))
      ctx.google.directory.users.push({
        googleUserId: `2000${String(i + 1).padStart(6, '0')}`,
        email: u.email,
