...
const RULES: Rule[] = [
  {
    ...
    resolve: (m) =>
      isValidDate(+m[1]!, +m[2]!, +m[3]!)
        ? { date: toIsoDate(+m[1]!, +m[2]!, +m[3]!), confidence: 0.98 }
        : null,
  },
  {
    ...
    resolve: (m) =>
      isValidDate(+m[3]!, +m[2]!, +m[1]!)
        ? { date: toIsoDate(+m[3]!, +m[2]!, +m[1]!), confidence: 0.95 }
        : null,
  },
  {
    regex: new RegExp(
      `\\b(?:el |del |para el |antes del |a mas tardar el |hasta el )?(\\d{1,2})(?:\\s*de\\s+|\\s+)(${MONTH_ALT})\\b(?:\\s+(?:de|del)\\s+(\\d{4}))?`,
    ),
    resolve: (m, ref) => {
  ...
  },
  {
    ...
    resolve: (_m, ref) => ({ date: addDays(ref, 2), confidence: 0.92 }),
  },
  {
    regex: /\bmanana\b(?! por la manana)/,
    resolve: (_m, ref) => ({ date: addDays(ref, 1), confidence: 0.9 }),
  },
  {
    regex: /\b(?:para hoy|hoy mismo|de hoy|antes de que termine el dia)\b/,
    resolve: (_m, ref) => ({ date: ref, confidence: 0.9 }),
  },
  {
    regex: new RegExp(
      `\\b(?:el |para el |antes del |a mas tardar el |hasta el |este |el proximo |el siguiente |proximo |siguiente )?(${WEEKDAY_ALT})(?:\\s+(?:proximo|siguiente|que viene|que entra))?\\b`,
    ),
    resolve: (m, ref) => {
  ...
  {
    regex: /\b(?:a |para |antes de |hasta )?(?:fin|finales|final) de(?:l)? mes\b/,
    resolve: (_m, ref) => ({ date: endOfMonth(ref), confidence: 0.8 }),
  },
  {
  ...
  {
    regex:
      /\b(?:la )?(?:proxima|siguiente) semana\b|\bla semana que viene\b|\bla semana que entra\b/,
    resolve: (_m, ref) => ({ date: nextWeekday(ref, 1), confidence: 0.5 }),
  },
  {
    regex:
      /\b(?:el |antes del |para el |hasta el )(?:dia )?(\d{1,2})\b(?:\s+de este mes)?(?!\s*(?:de|\/|-|:|%|por))/,
    resolve: (m, ref) => {
      const day = Number(m[1])
      const p = parseIsoDate(ref)
...
export function resolveRelativeDate(
  ...
  let best: {
    index: number
    result: { date: string; confidence: number }
    original: string
  } | null = null
  for (const rule of RULES) {
    ...
    if (
      !best ||
      m.index < best.index ||
      (m.index === best.index && result.confidence > best.result.confidence)
    ) {
