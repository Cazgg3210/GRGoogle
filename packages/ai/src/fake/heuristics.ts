...
const COMMITMENT_CUES: Array<{ regex: RegExp; strength: number }> = [
  ...
  {
    regex:
      /\byo (?:me encargo|lo hago|la hago|lo mando|la mando|lo envio|la envio|puedo|voy a|te (?:lo|la) (?:mando|envio|paso))\b/,
    strength: 0.2,
  },
  { regex: /\bquedamos en que\b/, strength: 0.15 },
  {
    regex:
      /\benviar[ae]?\b|\bmandar[ae]?\b|\bvoy a (?:enviar|mandar|preparar|revisar|coordinar|hacer|llamar|agendar|entregar)\b/,
    strength: 0.15,
  },
  {
    regex: /\bva a (?:enviar|mandar|preparar|revisar|coordinar|hacer|llamar|agendar|entregar)\b/,
    strength: 0.15,
  },
  ...
  {
    regex:
      /\b(?:revisar|preparar|mandar|enviar|coordinar|entregar|agendar|dar seguimiento|darle seguimiento|dale seguimiento|da seguimiento|doy seguimiento|seguimiento diario|seguimiento semanal)\b/,
    strength: 0.1,
  },
]

const DONE_CUES =
  /\b(?:ya quedo|ya esta listo|ya esta lista|ya quedo lista|ya quedo listo|ya lo mande|ya la mande|ya lo envie|ya la envie|ya esta (?:hecho|hecha|enviado|enviada|terminado|terminada)|terminado|terminada|ya se (?:envio|entrego|hizo))\b/
const BLOCKED_CUES =
  /\b(?:bloqueado|bloqueada|detenido|detenida|esperando a|en espera de|sigue bloqueado)\b/
const DECISION_CUES =
  /\b(?:se acordo|acordamos|quedamos en|se decide|se decidio|decidimos|queda acordado)\b/
...
function mentionedParticipant(
  sentenceFolded: string,
  ...
  for (const p of participants) {
    ...
    if (
      (full && sentenceFolded.includes(full)) ||
      (fn.length > 2 && new RegExp(`\\b${fn}\\b`).test(sentenceFolded))
    )
      return p
  }
  return null
...
function resolveOwner(
  ...
  if (mentioned) {
    return {
      owner: {
        ...
        ...(mentioned.email ? { email: mentioned.email } : {}),
        evidence: sentence,
      },
      ownerConfidenceBoost: 0.1,
    }
  ...
  for (const area of AREA_NAMES) {
    if (new RegExp(`\\b${area}\\b`).test(folded))
      return {
        owner: { name: area.charAt(0).toUpperCase() + area.slice(1), evidence: sentence },
        ownerConfidenceBoost: 0.05,
      }
  }
...
function buildTitle(sentence: string, dateText: string | null): string {
  ...
  t = t
    ...
    .replace(
      /^(?:hay que|tiene que|tienen que|debe|deben|va a|se compromete a|queda pendiente)\s+/i,
      '',
    )
    .replace(/^(?:que\s+)/i, '')
    .replace(/[¿?¡!]/g, '')
...
export function extractActionItems(input: AnalyzeMeetingInput): ExtractedActionItem[] {
  const participants: SpeakerContext[] = input.participants.map((p) => ({
  ...
  for (const seg of input.segments) {
    const speaker =
      bySpeaker.get(normalizeText(seg.speakerLabel)) ??
      (seg.speakerLabel ? { displayName: seg.speakerLabel, email: null, isInternal: false } : null)
    for (const sentence of splitSentences(seg.text)) {
      ...
      let strength = 0
      for (const cue of COMMITMENT_CUES)
        if (cue.regex.test(folded)) strength = Math.max(strength, cue.strength)
      ...
      const priority: ExtractedActionItem['priority'] = URGENT_CUES.test(folded)
        ? 'URGENT'
        : HIGH_CUES.test(folded)
          ? 'HIGH'
          : null
        ...
        evidence: [
          {
            ...
            ...(seg.startTime ? { startTime: seg.startTime } : {}),
            ...(seg.id ? { segmentId: seg.id } : {}),
          },
        ],
        confidence,
      ...
      if (dup) {
        if (
          item.confidence > dup.confidence ||
          (!dup.owner && item.owner) ||
          (!dup.dueDate && item.dueDate)
        ) {
  ...
  for (const seg of input.segments) {
    for (const sentence of splitSentences(seg.text)) {
      ...
      const description = sentence
        ...
        .replace(
          /^(?:se acordó|acordamos|quedamos en|se decide|se decidió|decidimos|queda acordado)\s+(?:que\s+)?/i,
          '',
        )
        .trim()
...
export function buildSummary(input: AnalyzeMeetingInput): MeetingAnalysisResult['summary'] {
  ...
  const executive = ordered
    .map((s) => splitSentences(s.text)[0] ?? s.text)
    .filter((s) => s.length > 0)
    .slice(0, 5)
  const risks: string[] = []
  ...
  for (const seg of input.segments) {
    for (const sentence of splitSentences(seg.text)) {
      ...
      if (/\?\s*$/.test(sentence) && /\bduda\b|\bpregunta\b|\bo esperamos\b/.test(folded))
  ...
  return {
    executive:
      executive.length > 0 ? executive : ['Reunión sin contenido suficiente para resumir.'],
...
export function heuristicAnalysis(input: AnalyzeMeetingInput): MeetingAnalysisResult {
  ...
  const avg =
    actionItems.length > 0
      ? actionItems.reduce((n, i) => n + i.confidence, 0) / actionItems.length
      : 0.5
  const totalChars = texts.reduce((n, t) => n + t.length, 0)
  const extractionConfidence = Math.min(
    0.95,
    Math.round((totalChars < 200 ? 0.4 : 0.6 + avg * 0.3) * 100) / 100,
  )
  const legal = /\bcontrato\b|\bjuridico\b|\bdemanda\b|\bpenalizaci/.test(
    foldForMatch(texts.join(' ')),
  )
  return {
