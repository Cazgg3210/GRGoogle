...
function participantByName(
  input: AnalyzeMeetingInput,
  needle: string,
): AnalyzeMeetingInput['participants'][number] | null {
...
export const clienteAlfaScenario: FakeScenario = {
  ...
  build(input) {
    ...
    return {
      ...
      topics: [
        {
          title: 'Contrato Cliente Alfa',
          subtopics: ['Anexo de penalizaciones', 'Carta de intención', 'Esquema de pagos'],
        },
      ...
      summary: {
        ...
        attentionPoints: [
          'La firma depende de que Jurídico no encuentre observaciones graves en el anexo.',
          'El onboarding requiere orden de compra del cliente.',
        ],
        risks: [
          'El área legal del cliente podría pedir cambios de última hora en las penalizaciones.',
        ],
        openQuestions: [
          '¿El onboarding de usuarios arranca con la firma o con la orden de compra?',
        ],
      },
      decisions: [
        {
          ...
          evidence: [
            {
              text: 'Entonces se acordó que el esquema de pagos se mantiene trimestral, como estaba en la propuesta original.',
              speaker: andres?.displayName ?? 'Andrés Escandón',
            },
          ],
          confidence: 0.92,
        },
        {
          description:
            'La firma del contrato con Cliente Alfa se hará antes de fin de mes, salvo observaciones graves de Jurídico.',
          decidedBy: null,
          effectiveDate: endOfMonth(ref),
          evidence: [
            {
              text: 'Quedamos en que la firma del contrato se hace antes de fin de mes, siempre que Jurídico no encuentre algo grave.',
              speaker: 'Lucía Ferrer',
            },
          ],
          confidence: 0.85,
        },
      ...
      actionItems: [
        {
          ...
          description:
            'Cliente Alfa enviará la carta de intención firmada por su dirección para liberar al equipo de implementación.',
          owner: {
            name: carlos?.displayName ?? 'Carlos Martínez',
            ...(carlos?.email ? { email: carlos.email } : {}),
            evidence:
              'yo voy a enviar la carta de intención firmada por nuestra dirección el próximo martes',
          },
          ...
          evidence: [
            {
              text: 'Por nuestra parte, yo voy a enviar la carta de intención firmada por nuestra dirección el próximo martes.',
              speaker: carlos?.displayName ?? 'Carlos Martínez',
            },
          ],
          confidence: 0.82,
        ...
        },
        {
          ...
          description:
            'Jurídico revisa el tope máximo de penalizaciones (15%) y entrega comentarios.',
          owner: {
            name: juridico?.displayName ?? 'Jurídico',
            ...(juridico?.email ? { email: juridico.email } : {}),
            evidence:
              'yo me encargo de que Jurídico revise el anexo de penalizaciones y tengamos comentarios para el viernes',
          },
          ...
          evidence: [
            {
              text: 'Lucía, ¿puedes coordinar con Jurídico que revisen el anexo de penalizaciones para el viernes?',
              speaker: andres?.displayName ?? 'Andrés Escandón',
            },
            {
              text: 'Sí, yo me encargo de que Jurídico revise el anexo de penalizaciones y tengamos comentarios para el viernes.',
              speaker: 'Lucía Ferrer',
            },
          ],
          confidence: 0.91,
        ...
        },
        {
          ...
          description:
            'El presupuesto de licencias con desglose por usuario ya fue enviado y validado por el cliente.',
          owner: {
          ...
          evidence: [
            {
              text: 'Sobre el presupuesto de licencias, ya quedó listo y lo mandé ayer al correo de Carlos con el desglose por usuario.',
              speaker: mariana?.displayName ?? 'Mariana Solís',
            },
          ],
          confidence: 0.86,
          relatedOpenActionKey: null,
...
export function findScenario(
  title: string,
  registry: FakeScenario[] = FAKE_SCENARIOS,
): FakeScenario | null {
