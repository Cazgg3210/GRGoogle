  ...
  actionItems: [
    {
      ...
      owner: { name: 'Carlos', evidence: 'yo envío' },
      dueDate: '2026-09-08',
      priority: null,
      statusHint: 'NEW',
      evidence: [{ text: 'yo envío la carta' }],
      confidence: 0.9,
    },
  ],
...
function client(
  handler: (
    ...
    call: number,
  ) => Promise<GenAiResponseLike> | GenAiResponseLike,
): GenAiClientLike & { calls: GenAiGenerateParams[] } {
...
describe('GeminiMeetingAnalyzer', () => {
  it('llama con structured output, valida y calcula usage/costo', async () => {
    ...
    expect(res.usage.estimatedCostUsd).toBeCloseTo(
      estimateCostUsd('gemini-2.5-flash', {
        inputTokens: 1000,
        outputTokens: 200,
        cachedTokens: 100,
      }) ?? -1,
    ...
    expect(
      estimateCostUsd('modelo-desconocido', { inputTokens: 1, outputTokens: 1, cachedTokens: 0 }),
    ).toBeNull()
  })
  ...
  it('reintenta con backoff en 429/503 y respeta el máximo', async () => {
    ...
    const ai = new GeminiMeetingAnalyzer({
      client: c,
      model: 'gemini-2.5-flash',
      maxRetries: 3,
      baseDelayMs: 100,
    ...
    const ai2 = new GeminiMeetingAnalyzer({
      client: always,
      model: 'gemini-2.5-flash',
      maxRetries: 1,
      ...fast,
    })
    await expect(ai2.analyzeMeeting(input())).rejects.toMatchObject({
    ...
    await expect(
      new GeminiMeetingAnalyzer({
        client: rate,
        model: 'x',
        maxRetries: 0,
        ...fast,
      }).analyzeMeeting(input()),
    ).rejects.toMatchObject({ code: 'GOOGLE_RATE_LIMIT' })
  })
  ...
  it('repara una salida inválida una vez y luego falla con AI_INVALID_OUTPUT', async () => {
    ...
    await expect(
      new GeminiMeetingAnalyzer({ client: bad, model: 'x', ...fast }).analyzeMeeting(input()),
  ...
  it('mapea timeout (abort) a error de proveedor reintentable', async () => {
    ...
    await expect(
      ai.reconcileActionItems({
        extracted: validAnalysis.actionItems[0]!,
        candidates: [],
        meetingTitle: 'R',
        referenceDate: '2026-09-03',
      }),
  ...
  it('hace chunking + consolidación en transcripciones largas y deduplica', async () => {
        ...
        return {
          text: JSON.stringify({
            ...
            actionItems: [
              ...
              { ...validAnalysis.actionItems[0], confidence: 0.7 },
            ],
          }),
        }
      }
    ...
    const ai = new GeminiMeetingAnalyzer({
      client: c,
      model: 'gemini-2.5-flash',
      chunkThresholdChars: 30_000,
      chunkSizeChars: 25_000,
      ...fast,
    })
  ...
  it('reconcile descarta ids que no son candidatos', async () => {
    const c = client(() => ({
      text: JSON.stringify({
        ...
        matchedActionItemId: 'otro',
        confidence: 0.9,
        rationale: 'x',
      }),
    }))
    const ai = new GeminiMeetingAnalyzer({ client: c, model: 'x', ...fast })
    const res = await ai.reconcileActionItems({
      extracted: validAnalysis.actionItems[0]!,
      candidates: [
        {
          actionItemId: 'a1',
  ...
  it('dedupeAnalysis fusiona por similitud de título', () => {
    const r = dedupeAnalysis({
      ...validAnalysis,
      actionItems: [
        validAnalysis.actionItems[0]!,
        {
          ...validAnalysis.actionItems[0]!,
          title: 'Enviar la carta de intención',
          confidence: 0.95,
          dueDate: null,
        },
      ],
