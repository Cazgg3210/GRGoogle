...
import { GoogleCalendarAdapter, extractMeetingCode, mapCalendarEvent, type CalendarApiClient } from './calendar.js'
...
describe('extractMeetingCode / mapCalendarEvent', () => {
  ...
  it('mapea un evento con conferenceData', () => {
    const e: calendar_v3.Schema$Event = {
      ...
      attendees: [{ email: 'lucia@smlxl.mx', responseStatus: 'accepted' }, { email: 'ext@cliente.example', organizer: false }],
      start: { dateTime: '2026-09-03T15:00:00-06:00', timeZone: 'America/Mexico_City' },
      end: { dateTime: '2026-09-03T16:00:00-06:00' },
      conferenceData: { entryPoints: [{ entryPointType: 'video', uri: 'https://meet.google.com/xyz-abcd-efg', meetingCode: 'XYZ-ABCD-EFG' }] },
...
describe('GoogleCalendarAdapter.syncEvents', () => {
  it('pagina y devuelve nextSyncToken', async () => {
    ...
    const client: CalendarApiClient = {
      events: {
        list: async (params) => {
          calls.push(params)
          if (!params.pageToken) return { data: { items: [{ id: 'a', hangoutLink: 'https://meet.google.com/abc-defg-hij', start: { dateTime: '2026-09-03T10:00:00Z' } }], nextPageToken: 'p2' } }
    ...
    const res = await adapter.syncEvents({ userEmail: 'andres@smlxl.mx', calendarId: 'primary', syncToken: null, timeMin: new Date('2026-08-01T00:00:00Z') })
  ...
  it('devuelve fullSyncRequired en 410', async () => {
    const client: CalendarApiClient = { events: { list: async () => { throw gone() } } }
    const adapter = new GoogleCalendarAdapter({ auth, clientFactory: () => client, retry: { retries: 0 } })
    const res = await adapter.syncEvents({ userEmail: 'andres@smlxl.mx', calendarId: 'primary', syncToken: 'old' })
  ...
  it('usa syncToken sin timeMin/timeMax', async () => {
    ...
    const client: CalendarApiClient = { events: { list: async (p) => { params = p; return { data: { items: [], nextSyncToken: 'tok-2' } } } } }
    const adapter = new GoogleCalendarAdapter({ auth, clientFactory: () => client })
    await adapter.syncEvents({ userEmail: 'andres@smlxl.mx', calendarId: 'primary', syncToken: 'tok-1', timeMin: new Date() })
  ...
  it('rechaza impersonar fuera del dominio', async () => {
    const adapter = new GoogleCalendarAdapter({ auth, clientFactory: () => ({ events: { list: async () => ({ data: {} }) } }) })
    await expect(adapter.syncEvents({ userEmail: 'x@otro.com', calendarId: 'primary', syncToken: null })).rejects.toThrow(/dominio/)
  })
})
