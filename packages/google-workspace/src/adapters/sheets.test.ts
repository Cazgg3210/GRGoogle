...
describe('GoogleSheetsAdapter', () => {
  it('crea la hoja si falta, actualiza por clave y agrega nuevas', async () => {
    ...
    const client: SheetsApiClient = {
      spreadsheets: {
        get: async () => {
          calls.push('get')
          return { data: { sheets: [{ properties: { title: 'Otra' } }] } }
        },
        batchUpdate: async (p) => {
          calls.push(`addSheet:${p.requestBody?.requests?.[0]?.addSheet?.properties?.title}`)
          return { data: {} }
        },
        values: {
          get: async () => {
          ...
          append: async (p) => {
            calls.push('append')
            appended = (p.requestBody?.values ?? []) as unknown[][]
            return { data: {} }
          },
          batchUpdate: async (p) => {
            calls.push('values.batchUpdate')
            batch.push(...(p.requestBody?.data ?? []))
            return { data: {} }
          },
        },
      },
    ...
    const adapter = new GoogleSheetsAdapter({
      auth,
      actingUserEmail: 'seguimiento@smlxl.mx',
  ...
  it('mapea errores a SHEETS_SYNC_FAILED', async () => {
    const client: SheetsApiClient = {
      spreadsheets: {
        get: async () => {
          const e = new Error('nope') as Error & { response: { status: number } }
    ...
    ).rejects.toMatchObject({ code: 'SHEETS_SYNC_FAILED' })
  })
})
