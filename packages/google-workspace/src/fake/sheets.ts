...
export class FakeSheetsAdapter implements SheetsPort {
  private readonly sheets = new Map<string, { columns: string[]; rows: Map<string, Record<string, string | number | boolean | null>> }>()
  ...
  async upsertRows(input: {
    ...
    for (const row of input.rows) {
      const values: Record<string, string | number | boolean | null> = { [input.keyColumn]: row.key }
  ...
  snapshot(): Record<string, FakeSheetSnapshot> {
    ...
    for (const [id, s] of this.sheets) out[id] = { columns: [...s.columns], rows: [...s.rows.values()].map((r) => ({ ...r })) }
    return out
  }
