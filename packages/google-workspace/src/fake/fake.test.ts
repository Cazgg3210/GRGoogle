...
describe('fake adapters', () => {
  it('carga las fixtures por defecto con la reunión de Cliente Alfa', () => {
    ...
    expect(alfa?.participants.map((p) => p.displayName)).toEqual([
      'Andrés Escandón',
      'Lucía Ferrer',
      'Carlos Martínez',
      'Mariana Solís',
  ...
  it('meet: expone records, participantes, entries y smart notes', async () => {
    const g = createFakeGoogleAdapters(env, {
      now,
      monitoredUserEmails: ['andres.escandon@smlxl.mx'],
    })
    const rec = await g.meet.getConferenceRecord(
      'conferenceRecords/fake-alfa-001',
      'andres.escandon@smlxl.mx',
    )
    ...
    expect(participants.find((p) => p.displayName === 'Carlos Martínez')?.email).toBe(
      'carlos.martinez@cliente-alfa.example',
    )
    const transcripts = await g.meet.listTranscripts(rec?.name ?? '', 'andres.escandon@smlxl.mx')
    const entries = await g.meet.listTranscriptEntries(
      transcripts[0]?.name ?? '',
      'andres.escandon@smlxl.mx',
    )
    expect(entries.length).toBeGreaterThan(20)
    expect(entries[0]?.participantName).toBe(
      'conferenceRecords/fake-alfa-001/participants/p-andres',
    )
    ...
    expect(
      await g.drive.exportDocumentText('doc-alfa-smartnotes-001', 'andres.escandon@smlxl.mx'),
    ).toContain('carta de intención')
  })

  it('meet: host externo no es accesible y patch bloqueado por política', async () => {
    const g = createFakeGoogleAdapters(env, { now })
    expect(
      await g.meet.listConferenceRecordsByMeetingCode('qrs-tuvw-xyz', 'lucia.ferrer@smlxl.mx'),
    ).toEqual([])
    const space = await g.meet.getSpaceByMeetingCode('mno-pqrs-tuv', 'lucia.ferrer@smlxl.mx')
    const res = await g.meet.patchArtifactConfig(
      space?.name ?? '',
      { autoTranscription: true, autoSmartNotes: true },
      'lucia.ferrer@smlxl.mx',
    )
    expect(res.applied).toBe(false)
    expect(res.blockedReason).toMatch(/política/)
    const ok = await g.meet.patchArtifactConfig(
      'spaces/fakeAlfaSpace01',
      { autoTranscription: true, autoSmartNotes: true },
      'andres.escandon@smlxl.mx',
    )
    expect(ok.applied).toBe(true)
    await expect(
      g.meet.getConferenceRecord('conferenceRecords/fake-alfa-001', 'x@otro.com'),
    ).rejects.toMatchObject({ code: 'GOOGLE_PERMISSION_DENIED' })
  })

  it('calendar: sync inicial + incremental + token inválido', async () => {
    const g = createFakeGoogleAdapters(env, { now })
    const first = await g.calendar.syncEvents({
      userEmail: 'lucia.ferrer@smlxl.mx',
      calendarId: 'primary',
      syncToken: null,
    })
    expect(first.events.map((e) => e.calendarEventId).sort()).toEqual([
      'cal-evt-beta-003',
      'cal-evt-pipeline-002',
    ])
    expect(first.events.find((e) => e.calendarEventId === 'cal-evt-beta-003')?.organizerEmail).toBe(
      'ana.lopez@proveedor-beta.example',
    )
    const second = await g.calendar.syncEvents({
      userEmail: 'lucia.ferrer@smlxl.mx',
      calendarId: 'primary',
    ...
    g.calendar.addEvent({
      userEmail: 'lucia.ferrer@smlxl.mx',
      calendarEventId: 'new-1',
      title: 'Nueva',
      organizerEmail: 'lucia.ferrer@smlxl.mx',
      attendees: [],
    ...
    const third = await g.calendar.syncEvents({
      userEmail: 'lucia.ferrer@smlxl.mx',
      calendarId: 'primary',
    ...
    const fourth = await g.calendar.syncEvents({
      userEmail: 'lucia.ferrer@smlxl.mx',
      calendarId: 'primary',
      syncToken: third.nextSyncToken,
    })
  ...
  it('workspace events, directory, mail y sheets', async () => {
    const g = createFakeGoogleAdapters(env, {
      now,
      monitoredUserEmails: ['andres.escandon@smlxl.mx', 'lucia.ferrer@smlxl.mx'],
    })
    ...
    const sub = await g.workspaceEvents.createUserSubscription({
      userEmail: 'andres.escandon@smlxl.mx',
      userResourceName: rn ?? '',
      eventTypes: [GoogleMeetEventType.CONFERENCE_ENDED],
      pubsubTopic: 'projects/p/topics/t',
    })
    expect(sub.expiresAt.getTime() - now().getTime()).toBe(7 * 86_400_000)
    expect(
      (await g.workspaceEvents.getSubscription(sub.subscriptionName, 'andres.escandon@smlxl.mx'))
        ?.state,
    ).toBe('ACTIVE')
    const m1 = await g.mail.send({
      to: ['a@smlxl.mx'],
      subject: 'x',
      html: '<p>x</p>',
      text: 'x',
    ...
    await g.sheets.upsertRows({
      spreadsheetId: 's',
      sheetName: 'Pendientes',
      keyColumn: 'UUID',
      columns: ['UUID', 'Estado'],
      rows: [{ key: 'a', values: { Estado: 'PENDING' } }],
    })
    await g.sheets.upsertRows({
      spreadsheetId: 's',
      sheetName: 'Pendientes',
      ...
      columns: ['UUID', 'Estado'],
      rows: [
        { key: 'a', values: { Estado: 'COMPLETED' } },
        { key: 'b', values: { Estado: 'PENDING' } },
      ],
    })
    const snap = g.sheets.snapshot()['s/Pendientes']
    expect(snap?.rows).toEqual([
      { UUID: 'a', Estado: 'COMPLETED' },
      { UUID: 'b', Estado: 'PENDING' },
    ])
  })

  it('createFakeConferenceEndedEvent produce un CloudEvent por meetingCode', () => {
    const ev = createFakeConferenceEndedEvent('abc-defg-hij', {
      subscribedUserEmail: 'andres.escandon@smlxl.mx',
    })
