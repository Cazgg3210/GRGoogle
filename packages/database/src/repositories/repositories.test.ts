...
import { createPrismaClient, createRepositories, PrismaUnitOfWork, type PrismaClient, type RepositoryDefaults } from '../index.js'
...
describe.skipIf(!DATABASE_URL)('Repositorios Prisma (integración)', () => {
  ...
  it('crea action items con externalKey secuencial único y colaboradores', async () => {
    const a = await repos.actionItems.save(makeItem({ title: `${TAG} tarea A`, ownerUserId: owner.id }))
    ...
    const updated = await repos.actionItems.save({ ...b, collaboratorUserIds: [], title: `${TAG} tarea B2` })
  ...
  it('full-text en español encuentra por raíz y respeta openOnly', async () => {
    const open = await repos.actionItems.save(
      makeItem({ title: `${TAG} Revisar anexo de penalizaciones del contrato`, status: ActionItemStatus.PENDING }),
    )
    ...
    const all = await repos.actionItems.searchFullText('penalizaciones contrato', { openOnly: false, limit: 20 })
    const allIds = all.map((i) => i.id)
    expect(allIds).toContain(open.id)
    expect(allIds).toContain(closed.id)

    const onlyOpen = await repos.actionItems.searchFullText('penalizaciones contrato', { openOnly: true, limit: 20 })
  ...
  it('list aplica overdueOnly, noOwner, noDueDate y visibleToUserId', async () => {
    ...
    const future = await repos.actionItems.save(makeItem({ title: `${TAG} futura`, dueDate: tomorrow }))
    const done = await repos.actionItems.save(
      makeItem({ title: `${TAG} cerrada vencida`, dueDate: yesterday, status: ActionItemStatus.COMPLETED }),
    )

    const overdueList = await repos.actionItems.list({ tags: [TAG], overdueOnly: true }, { page: 1, pageSize: 50 })
    const ids = overdueList.items.map((i) => i.id)
    expect(ids).toContain(overdue.id)
    expect(ids).not.toContain(future.id)
    expect(ids).not.toContain(done.id)

    const noOwner = await repos.actionItems.list({ tags: [TAG], noOwner: true }, { page: 1, pageSize: 50 })
    expect(noOwner.items.every((i) => i.ownerUserId === null && i.externalAssigneeId === null)).toBe(true)
    expect(noOwner.items.map((i) => i.id)).toContain(future.id)

    const noDue = await repos.actionItems.list({ tags: [TAG], noDueDate: true }, { page: 1, pageSize: 50 })
  ...
  it('filtra reuniones por participante, processed, withActionItems y search', async () => {
    const withItems = await repos.meetings.save(
      makeMeeting({ title: `${TAG} Seguimiento contrato`, startAt: new Date(Date.now() - 3600_000) }),
    )
    ...
    const processed = await repos.meetings.list({ processed: true, search: TAG }, { page: 1, pageSize: 20 })
    ...
    const visible = await repos.meetings.list({ visibleToUserId: owner.id, search: TAG }, { page: 1, pageSize: 20 })
  ...
  it('la unidad de trabajo revierte todo si la función lanza', async () => {
    ...
    const after = await repos.actionItems.list({ tags: [TAG], search: 'rollback' }, { page: 1, pageSize: 10 })
    expect(after.items.find((i) => i.title === title)).toBeUndefined()

    const created = await uow.run(async (tx) => tx.actionItems.save(makeItem({ title: `${TAG} commit` })))
  ...
  it('settings mezcla defaults del entorno con overrides de BD', async () => {
    ...
    expect(settings.confidenceThresholds.autoAccept).toBeGreaterThan(settings.confidenceThresholds.proposal)
  })
})
