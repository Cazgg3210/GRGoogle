...
export class PrismaProcessingRunRepository extends BaseRepository implements ProcessingRunRepository {
  ...
  async listByMeeting(meetingId: Id): Promise<ProcessingRun[]> {
    const rows = await this.db.processingRun.findMany({ where: { meetingId }, orderBy: { startedAt: 'desc' } })
    return rows.map(toProcessingRun)
  }

  async save(run: ProcessingRun): Promise<ProcessingRun> {
    const { id, ...rest } = processingRunToDb(run)
    const row = await this.db.processingRun.upsert({ where: { id: run.id }, create: { id, ...rest }, update: rest })
    return toProcessingRun(row)
  }

  /** Consumo/costo IA en el periodo (§35). */
  async usageSummary(from: Date, to: Date): Promise<{
    runs: number
    inputTokens: number
    outputTokens: number
