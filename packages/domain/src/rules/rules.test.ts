...
import { detectRecurrenceHint, normalizePriority, normalizeText, tokenJaccard, trigramSimilarity } from './normalize.js'
...
describe('attention score', () => {
  ...
  it('vencida + alta prioridad pesa más que todo', () => {
    const r = attentionScore({ item: { ...base, priority: 'HIGH', dueDate: parseLocalDate('2026-09-01')! } }, now)
    expect(r.reasons[0]).toBe(AttentionReason.OVERDUE_HIGH_PRIORITY)
    expect(r.score).toBeGreaterThan(attentionScore({ item: { ...base, ownerUserId: null, dueDate: null } }, now).score)
  })
  it('completadas no requieren atención', () => {
    expect(attentionScore({ item: { ...base, status: 'COMPLETED' } }, now).score).toBe(0)
  })
  it('explica razones', () => {
    const r = attentionScore({ item: { ...base, ownerUserId: null, dueDate: null, confidence: 0.5 } }, now)
    expect(r.reasons).toEqual([AttentionReason.NO_OWNER, AttentionReason.NO_DUE_DATE, AttentionReason.LOW_AI_CONFIDENCE])
  })
})
...
describe('normalización', () => {
  ...
  it('similitud entre variantes', () => {
    ...
    expect(tokenJaccard('Enviar carta al cliente Alfa', 'Enviar la carta a cliente Alfa')).toBeGreaterThan(0.7)
...
describe('RBAC', () => {
  ...
  const manager = { id: 'g', role: UserRole.MANAGER, areaId: 'a1', email: 'g@smlxl.mx', teamUserIds: ['m'] }
  it('permisos por rol', () => {
  ...
  it('MEMBER sólo ve reuniones donde participa', () => {
    const meeting = { organizerUserId: 'x', confidentialityLevel: ConfidentialityLevel.NORMAL, areaId: 'a1', participantUserIds: ['y'] }
  ...
  it('MANAGER ve reuniones de su área/equipo, salvo restringidas', () => {
    const meeting = { organizerUserId: 'm', confidentialityLevel: ConfidentialityLevel.NORMAL, areaId: 'a1', participantUserIds: [] as string[] }
    expect(canAccessMeeting(manager, meeting)).toBe(true)
    expect(canAccessMeeting(manager, { ...meeting, confidentialityLevel: ConfidentialityLevel.LEGAL })).toBe(false)
  })
...
describe('calidad de captura', () => {
  it('clasifica host externo sin artefactos', () => {
    const b = captureQualityBuckets({ transcriptStatus: 'UNAVAILABLE_EXTERNAL_HOST', smartNotesStatus: 'UNAVAILABLE_EXTERNAL_HOST', processingStatus: 'COMPLETED', isExternalHost: true })
    expect(b).toContain('EXTERNAL_HOST_UNAVAILABLE')
    expect(b).not.toContain('NO_ARTIFACT')
  })
  it('sólo transcript', () => {
    const b = captureQualityBuckets({ transcriptStatus: 'INGESTED', smartNotesStatus: 'UNAVAILABLE', processingStatus: 'COMPLETED', isExternalHost: false })
