...
describe('handlers de jobs', () => {
  ...
  it('rechaza payloads inválidos con ZodError', async () => {
    await expect(queue.enqueue(JobNames.FETCH_MEETING_ARTIFACTS, {})).rejects.toBeInstanceOf(
      ZodError,
    )
    await expect(
      queue.enqueue(JobNames.SEND_WEEKLY_DIGEST, { digestId: 42 }),
    ).rejects.toBeInstanceOf(ZodError)
    await expect(
      queue.enqueue(JobNames.PROCESS_GOOGLE_EVENT, { event: { id: 'x' } }),
    ).rejects.toBeInstanceOf(ZodError)
  })

  it('un fallo de FETCH_MEETING_ARTIFACTS se relanza para que la cola reintente', async () => {
    await expect(
      queue.enqueue(JobNames.FETCH_MEETING_ARTIFACTS, {
  ...
  it('jobs de cron con payload vacío/undefined se aceptan y respetan feature flags', async () => {
    ...
    await expect(
      queue.enqueue(JobNames.RECONCILE_ACTION_ITEMS, { meetingId: 'm', processingRunId: 'r' }),
    ).resolves.toBeTruthy()
  ...
  it('GENERATE_WEEKLY_DIGEST genera el digest y omite el envío si está deshabilitado', async () => {
    ...
    await expect(
      queue.enqueue(JobNames.GENERATE_WEEKLY_DIGEST, { sendAfterGenerate: true }),
    ).resolves.toBeTruthy()
...
describe('programación de cron', () => {
  it('registra los cron fijos con la zona horaria de la empresa y omite Calendar si la integración está apagada', async () => {
    ...
    expect(names).toEqual(
      expect.arrayContaining([
        JobNames.RENEW_GOOGLE_SUBSCRIPTIONS,
        JobNames.SEND_DUE_REMINDERS,
        JobNames.CLEANUP_EXPIRED_RAW_DATA,
        JobNames.RETRY_FAILED_MEETING_PROCESSING,
        JobNames.RECONCILE_MISSING_EVENTS,
      ]),
    )
    expect(names).not.toContain(JobNames.CALENDAR_INCREMENTAL_SYNC)
    expect(summary.skipped).toEqual([
      { name: JobNames.CALENDAR_INCREMENTAL_SYNC, reason: 'GOOGLE_INTEGRATION_ENABLED=false' },
    ])
    expect(queue.scheduled.find((s) => s.name === JobNames.SEND_DUE_REMINDERS)?.cron).toBe(
      CRON[JobNames.SEND_DUE_REMINDERS],
    )
    ...
    expect(queue.scheduled.find((s) => s.name === JobNames.GENERATE_WEEKLY_DIGEST)?.cron).toBe(
      summary.digest.cron,
    )
  })
  ...
  it('reprograma el digest sólo cuando cambia la configuración', async () => {
    ...
    const cfg = await ctx.repos.digests.getConfig()
    await ctx.repos.digests.saveConfig({
      ...cfg,
      dayOfWeek: (cfg.dayOfWeek + 1) % 7,
      localTime: '09:30',
    })
    const changed = await applyDigestSchedule(queue, application, logger, first)
    expect(changed.cron).not.toBe(first.cron)
    expect(queue.scheduled.at(-1)).toMatchObject({
      name: JobNames.GENERATE_WEEKLY_DIGEST,
      cron: changed.cron,
    })
  })
})
