...
export function registerAdminRoutes(app: AppServer, deps: RouteDeps): void {
  ...
  app.get(
    '/api/v1/admin/users',
    { schema: { tags, response: { 200: z.array(UserDtoSchema) } }, preHandler: users },
    async () => application.admin.listUsers(),
  )
  app.patch(
  ...
  app.get(
    '/api/v1/admin/areas',
    { schema: { tags, response: { 200: z.array(AreaDtoSchema) } }, preHandler: catalogs },
    async () => application.admin.listAreas(false),
  )
  app.post(
    '/api/v1/admin/areas',
    {
      schema: { tags, body: UpsertAreaBodySchema, response: { 201: AreaDtoSchema } },
      preHandler: catalogs,
    },
    async (request, reply) =>
      reply
        .status(201)
        .send(await application.admin.upsertArea(requirePrincipal(request), request.body)),
  )
  app.patch(
    '/api/v1/admin/areas/:id',
    {
      schema: {
        tags,
        params: IdParams,
        body: UpsertAreaBodySchema,
        response: { 200: AreaDtoSchema },
      },
      preHandler: catalogs,
    },
    async (request) =>
  ...
  app.get(
    ...
    { schema: { tags, response: { 200: z.array(ProjectDtoSchema) } }, preHandler: catalogs },
    async () => application.admin.listProjects(false),
  )
  app.post(
    '/api/v1/admin/projects',
    {
      schema: { tags, body: UpsertProjectBodySchema, response: { 201: ProjectDtoSchema } },
      preHandler: catalogs,
    },
    async (request, reply) =>
      reply
        .status(201)
        .send(await application.admin.upsertProject(requirePrincipal(request), request.body)),
  )
  app.patch(
  ...
  app.get(
    ...
    { schema: { tags, response: { 200: PlatformSettingsDtoSchema } }, preHandler: config },
    async (request) => application.admin.getPlatformSettings(requirePrincipal(request)),
  )
  app.put(
    '/api/v1/admin/settings',
    {
      schema: {
        tags,
        body: UpdatePlatformSettingsBodySchema,
        response: { 200: PlatformSettingsDtoSchema },
      },
      preHandler: config,
    },
    async (request) =>
  ...
  app.get(
    '/api/v1/admin/audit',
    {
      schema: {
        ...
        response: { 200: pageSchema(AuditEntryDtoSchema) },
      },
  ...
  app.get(
    '/api/v1/admin/jobs',
    {
      schema: { tags, response: { 200: JobQueueStatsSchema } },
      preHandler: requirePermission(Permission.INTEGRATION_MANAGE),
    },
    async () => {
