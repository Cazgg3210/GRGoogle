import { AiReviewApproveBodySchema, AiReviewItemDtoSchema, AiReviewListQuerySchema, AiReviewMergeBodySchema, AiReviewRejectBodySchema, pageSchema } from '@smlxl/contracts'
...
export function registerAiReviewRoutes(app: AppServer, deps: RouteDeps): void {
  ...
  app.get(
    '/api/v1/ai-review',
    { schema: { tags, querystring: AiReviewListQuerySchema, response: { 200: pageSchema(AiReviewItemDtoSchema) } }, preHandler: resolve },
    async (request) => application.aiReview.list(requirePrincipal(request), request.query),
  )

  app.post(
    '/api/v1/ai-review/:id/approve',
    { schema: { tags, params: IdParams, body: AiReviewApproveBodySchema.optional(), response: { 200: AiReviewItemDtoSchema } }, preHandler: resolve },
    async (request) => {
  ...
  app.post(
    ...
    { schema: { tags, params: IdParams, body: AiReviewRejectBodySchema.optional(), response: { 200: AiReviewItemDtoSchema } }, preHandler: resolve },
    async (request) => {
      const principal = requirePrincipal(request)
  ...
  app.post(
    ...
    { schema: { tags, params: IdParams, body: AiReviewMergeBodySchema, response: { 200: AiReviewItemDtoSchema } }, preHandler: resolve },
    async (request) => {
      const principal = requirePrincipal(request)
