...
import {
  ...
  type SeededUsers,
  type TestContext,
} from '@smlxl/application/testing'
...
async function buildTestApp(
  ...
  const base = createTestContext({
    env: {
      ...
      AUTH_SECRET: SECRET,
      GOOGLE_PUBSUB_PUSH_TOKEN: PUSH_TOKEN,
      ...env,
    },
  })
  ...
  registerRoutes(app, {
    ...
    jobStats: async () => [{ name: 'q', created: 1, active: 0, completed: 0, failed: 0 }],
  })
  await app.ready()
...
function pubsubBody(event: unknown, mode: 'structured' | 'binary' = 'structured') {
  if (mode === 'structured') {
    return {
      message: { messageId: 'm-1', data: Buffer.from(JSON.stringify(event)).toString('base64') },
  ...
  return {
    message: {
      ...
      attributes: {
        ...
        'ce-type': e.type,
        'ce-source': e.source,
        ...(e.subject ? { 'ce-subject': e.subject } : {}),
        ...(e.time ? { 'ce-time': e.time } : {}),
      },
    },
    subscription: 'projects/p/subscriptions/s',
  }
...
describe('autenticación y sesión', () => {
  ...
  it('bypass por header resuelve el usuario por email y devuelve permisos efectivos', async () => {
    ...
    expect(body.user).toMatchObject({
      id: t.users.andres.id,
      email: t.users.andres.email,
      role: 'DIRECTOR',
  ...
  it('JWT válido (sub = email, como el token de arranque de la web) autentica; token inválido → 401', async () => {
    const token = await mintApiToken(
      { sub: t.users.mariana.email, email: t.users.mariana.email, role: 'MEMBER', name: 'Mariana' },
      SECRET,
    )
    ...
    const wrongSecret = await mintApiToken(
      { sub: 'x', email: t.users.mariana.email, role: 'MEMBER', name: 'M' },
      'otro-secreto-123',
    )
    expect(
      (
  ...
  it('/health es público y /openapi.json expone las rutas', async () => {
    expect((await t.app.inject({ method: 'GET', url: '/health' })).json()).toMatchObject({
      status: 'ok',
      db: 'up',
      version: 'test',
    })
...
describe('RBAC y formato de errores', () => {
  it('MEMBER recibe 403 en rutas de administración; ADMIN 200', async () => {
    const forbidden = await t.app.inject({
      ...
      headers: asUser(t.users.mariana.email),
    })
    ...
    ).toEqual({ queues: [{ name: 'q', created: 1, active: 0, completed: 0, failed: 0 }] })
  })

  it('404 de dominio, 422 de validación y 404 de ruta usan ErrorResponseSchema', async () => {
    const notFound = await t.app.inject({
      method: 'GET',
      url: '/api/v1/meetings/00000000-0000-4000-8000-000000009999',
      headers: asUser(t.users.andres.email),
    })
    ...
    const invalid = await t.app.inject({
      method: 'GET',
      url: '/api/v1/action-items?page=0',
      headers: asUser(t.users.andres.email),
    })
...
describe('webhook Pub/Sub', () => {
  ...
  it('rechaza sin token o con token incorrecto', async () => {
    expect(
      (
        await t.app.inject({
          method: 'POST',
          url: '/api/v1/webhooks/google/pubsub',
          payload: pubsubBody(event),
        })
      ).statusCode,
    ).toBe(401)
    expect(
      (
        await t.app.inject({
          method: 'POST',
          url: '/api/v1/webhooks/google/pubsub?token=wrong',
          payload: pubsubBody(event),
        })
      ).statusCode,
    ).toBe(401)
  ...
  it('rechaza siempre cuando GOOGLE_PUBSUB_PUSH_TOKEN está vacío', async () => {
    ...
    const res = await other.app.inject({
      method: 'POST',
      url: '/api/v1/webhooks/google/pubsub?token=',
      payload: pubsubBody(event),
    })
    expect(res.statusCode).toBe(401)
  ...
  it('procesa el evento (204), es idempotente ante duplicados (204) y crea la reunión', async () => {
    const first = await t.app.inject({
      method: 'POST',
      url: `/api/v1/webhooks/google/pubsub?token=${PUSH_TOKEN}`,
      payload: pubsubBody(event),
    })
    ...
    const second = await t.app.inject({
      method: 'POST',
      url: `/api/v1/webhooks/google/pubsub?token=${PUSH_TOKEN}`,
      payload: pubsubBody(event, 'binary'),
    })
  ...
  it('mensajes sin CloudEvent interpretable se confirman con 204', async () => {
    const res = await t.app.inject({
      ...
      url: `/api/v1/webhooks/google/pubsub?token=${PUSH_TOKEN}`,
      payload: {
        message: { messageId: 'm-x', data: Buffer.from('no json').toString('base64') },
        subscription: 's',
      },
    })
  ...
  it('decodifica binary y structured content mode', () => {
    expect(cloudEventFromPubSub(pubsubBody(event))).toMatchObject({
      id: 'evt-pubsub-1',
      type: event.type,
    })
    expect(cloudEventFromPubSub(pubsubBody(event, 'binary'))).toMatchObject({
      id: 'evt-pubsub-1',
      type: event.type,
      source: event.source,
    })
...
describe('flujo de action items por HTTP', () => {
  it('crear → proponer cierre → aprobar; MEMBER no puede aprobar', async () => {
    ...
    const proposed = await t.app.inject({
      method: 'POST',
      url: `/api/v1/action-items/${item.id}/complete`,
      headers: asUser(t.users.lucia.email),
      payload: { reason: 'Contrato enviado y confirmado' },
    })
    ...
    const denied = await t.app.inject({
      method: 'POST',
      url: `/api/v1/action-items/${item.id}/proposals/${proposalId}/approve`,
      headers: asUser(t.users.mariana.email),
      payload: {},
    })
    expect(denied.statusCode).toBe(403)

    const counts = await t.app.inject({
      method: 'GET',
    ...
    expect(await t.ctx.repos.completionProposals.findById(proposalId)).toMatchObject({
      status: 'APPROVED',
      reviewComment: 'ok',
      reviewedByUserId: t.users.andres.id,
    })

    const comment = await t.app.inject({
      method: 'POST',
      url: `/api/v1/action-items/${item.id}/comments`,
      headers: asUser(t.users.lucia.email),
    ...
    const list = await t.app.inject({
      method: 'GET',
      url: '/api/v1/action-items?view=completed',
      headers: asUser(t.users.andres.email),
    })
...
describe('revisión IA por HTTP', () => {
  it('lista (con filtro reason) y aprueba una propuesta', async () => {
    ...
    const review = await t.ctx.repos.aiReview.save({
      ...
      candidateScore: null,
      proposedActionItemId: proposedId,
      extracted: {
        title: 'Tarea IA',
        owner: null,
        dueDate: null,
    ...
    expect(all.json()).toMatchObject({
      total: 1,
      items: [{ id: review.id, reasons: ['LOW_CONFIDENCE'] }],
    })
    expect(
      (
        await t.app.inject({
          method: 'GET',
          url: '/api/v1/ai-review?reason=POSSIBLE_COMPLETION',
          headers: asUser(t.users.andres.email),
        })
      ).json().total,
    ).toBe(0)
    expect(
      (
        await t.app.inject({
          method: 'GET',
          url: '/api/v1/ai-review?reason=LOW_CONFIDENCE',
          headers: asUser(t.users.andres.email),
        })
      ).json().total,
    ).toBe(1)
    expect(
      ...
      ).json().pendingAiReview,
    ).toBe(1)

    const approved = await t.app.inject({
      method: 'POST',
      url: `/api/v1/ai-review/${review.id}/approve`,
      headers: asUser(t.users.andres.email),
      payload: { ownerUserId: t.users.lucia.id, dueDate: '2026-09-12' },
    })
    expect(approved.statusCode).toBe(200)
    expect(approved.json()).toMatchObject({ id: review.id, status: 'APPROVED' })
    const detail = await t.app.inject({
      method: 'GET',
      url: `/api/v1/action-items/${proposedId}`,
      headers: asUser(t.users.andres.email),
    })
...
describe('integraciones', () => {
  it('simulate/meeting-ended ejecuta el pipeline FAKE y la reunión queda analizada', async () => {
    const res = await t.app.inject({
      method: 'POST',
      url: '/api/v1/integrations/simulate/meeting-ended',
      headers: asUser(t.users.admin.email),
      payload: {},
    })
    expect(res.statusCode).toBe(200)
    ...
    const transcript = await t.app.inject({
      method: 'GET',
  ...
  it('simulate responde 409 FEATURE_DISABLED fuera del modo FAKE', async () => {
    const real = await buildTestApp({
      ...
      GOOGLE_SERVICE_ACCOUNT_EMAIL: 'svc@smlxl.mx',
    })
    const res = await real.app.inject({
      method: 'POST',
      url: '/api/v1/integrations/simulate/meeting-ended',
      headers: asUser(real.users.admin.email),
      payload: {},
    })
  ...
  it('calendar/sync encola CALENDAR_INCREMENTAL_SYNC y sheets/sync dryRun devuelve la vista previa', async () => {
    ...
    const sheets = await t.app.inject({
      method: 'POST',
  ...
  it('catálogos de equipo incluyen responsables externos', async () => {
    await t.ctx.repos.externalAssignees.save({
      id: t.base.ids.next(),
    ...
    const res = await t.app.inject({
      method: 'GET',
      url: '/api/v1/team/external-assignees',
      headers: asUser(t.users.mariana.email),
    })
    expect(res.statusCode).toBe(200)
