...
function KpiTable({
  ...
  rows: Row[]
  entity: 'área' | 'persona'
  linkParam: 'areaId' | 'ownerUserId'
}) {
  if (rows.length === 0) {
    return (
      <EmptyState
        compact
        title={`Sin datos por ${entity}`}
        description="No hay pendientes en el período/filtros seleccionados."
      />
    )
  }
    ...
    <Table>
      ...
      <TableBody>
        {rows.map((r) => (
          <TableRow key={r.key}>
            <TableCell className="font-medium">
              <Link
                href={`/pendientes?${linkParam}=${encodeURIComponent(r.key)}`}
            ...
            <TableCell className="text-right font-mono text-xs tabular">
              {formatNumber(r.inProgress)}
            </TableCell>
            <TableCell className="text-right font-mono text-xs tabular">
              {formatNumber(r.pending)}
            </TableCell>
            {entity === 'área' ? (
              <TableCell className="text-right font-mono text-xs tabular text-signal-700">
                {formatNumber(r.completionProposed)}
              </TableCell>
            ) : null}
            <TableCell
              className={cn(
                'text-right font-mono text-xs tabular',
                r.overdue > 0 && 'font-semibold text-danger-700',
              )}
            >
            ...
            <TableCell>
              <div className="flex items-center gap-2">
                <Progress
                  value={r.progressPct}
                  tone={
                    r.progressPct >= 70 ? 'success' : r.progressPct >= 40 ? 'warning' : 'danger'
                  }
                  className="flex-1"
                />
                <span className="w-9 text-right font-mono text-xs tabular">
        ...
        <TableRow className="bg-surface-muted/60 font-semibold hover:bg-surface-muted/60">
          ...
          <TableCell className="text-right font-mono text-xs tabular">
            {formatNumber(totals.inProgress)}
          </TableCell>
          <TableCell className="text-right font-mono text-xs tabular">
          ...
          {entity === 'área' ? (
            <TableCell className="text-right font-mono text-xs tabular">
              {formatNumber(totals.completionProposed)}
            </TableCell>
          ) : null}
          <TableCell className="text-right font-mono text-xs tabular">
  ...
  return (
    <Card>
      <CardHeader className="flex-row flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {periodLabel}. Las áreas provienen del catálogo; “Externos” es una categoría especial.
          </p>
        </div>
      ...
      <CardContent className="px-0 pb-0">
        {tab === 'area' ? (
          <KpiTable rows={byArea} entity="área" linkParam="areaId" />
        ) : (
          <KpiTable rows={byPerson} entity="persona" linkParam="ownerUserId" />
        )}
      </CardContent>
    </Card>
  )
