...
export function CatalogsAdmin({ areas, projects }: { areas: AreaDto[]; projects: ProjectDto[] }) {
  ...
  const [projectDialog, setProjectDialog] = React.useState<null | { project: ProjectDto | null }>(null)
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
                ...
                .map((a) => (
                  <TableRow key={a.id}>
                    <TableCell className="font-medium">
                      {a.name}
                      {a.isExternalCategory ? <Badge tone="warning" className="ml-2">Externos</Badge> : null}
                    </TableCell>
            ...
            <TableBody>
              {projects.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="xs" onClick={() => setProjectDialog({ project: p })}>
                      <Pencil />
                    </Button>
                  </TableCell>
                </TableRow>
      ...
      {areaDialog ? <AreaDialog area={areaDialog.area} onClose={() => setAreaDialog(null)} /> : null}
      {projectDialog ? <ProjectDialog project={projectDialog.project} areas={areas} onClose={() => setProjectDialog(null)} /> : null}
    </div>
  )
}

function AreaDialog({ area, onClose }: { area: AreaDto | null; onClose: () => void }) {
  const [form, setForm] = React.useState({ name: area?.name ?? '', code: area?.code ?? '', active: area?.active ?? true, sortOrder: area?.sortOrder ?? 100 })
  const save = useApiMutation<AreaDto, typeof form>({
    mutationFn: (f) => {
      const body = { name: f.name.trim(), code: f.code.trim() || null, active: f.active, sortOrder: f.sortOrder }
      return area ? clientApi.patch<AreaDto>(`/admin/areas/${area.id}`, body) : clientApi.post<AreaDto>('/admin/areas', body)
    },
      ...
      <DialogContent size="sm">
        ...
        <div className="flex flex-col gap-4">
          <Field label="Nombre" htmlFor="a-name" required>
            <Input id="a-name" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
          </Field>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Código" htmlFor="a-code">
              <Input id="a-code" value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} maxLength={20} />
            </Field>
            <Field label="Orden" htmlFor="a-order">
              <Input id="a-order" type="number" value={form.sortOrder} onChange={(e) => setForm((f) => ({ ...f, sortOrder: Number(e.target.value) }))} />
            </Field>
          </div>
...
function ProjectDialog({ project, areas, onClose }: { project: ProjectDto | null; areas: AreaDto[]; onClose: () => void }) {
  ...
  const save = useApiMutation<ProjectDto, typeof form>({
    mutationFn: (f) => {
      ...
      return project ? clientApi.patch<ProjectDto>(`/admin/projects/${project.id}`, body) : clientApi.post<ProjectDto>('/admin/projects', body)
    },
      ...
      <DialogContent>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Nombre canónico" htmlFor="p-name" required className="sm:col-span-2">
            <Input id="p-name" value={form.canonicalName} onChange={(e) => setForm((f) => ({ ...f, canonicalName: e.target.value }))} />
          </Field>
          <Field label="Código" htmlFor="p-code">
            <Input id="p-code" value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} maxLength={30} />
          </Field>
          ...
          <Field label="Alias" htmlFor="p-alias" hint="Uno por línea o separados por coma. La IA los usa para reconocer el proyecto en las transcripciones." className="sm:col-span-2">
            <Textarea id="p-alias" rows={4} value={form.aliases} onChange={(e) => setForm((f) => ({ ...f, aliases: e.target.value }))} />
          </Field>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => save.mutate(form)} loading={save.isPending} disabled={!form.canonicalName.trim()}>
            Guardar
          </Button>
        </DialogFooter>
      </DialogContent>
