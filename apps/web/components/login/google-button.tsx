...
function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" className="size-4" aria-hidden>
      <path
        fill="#4285F4"
        d="M23.5 12.3c0-.8-.1-1.6-.2-2.3H12v4.4h6.5c-.3 1.5-1.1 2.8-2.4 3.6v3h3.9c2.2-2.1 3.5-5.1 3.5-8.7z"
      />
      <path
        fill="#34A853"
  ...
  return (
    <form action={googleSignInAction} className="flex flex-col gap-2">
      ...
      <Button
        ...
        size="lg"
        variant="outline"
        className="w-full justify-center gap-3 bg-surface"
        disabled={!enabled}
      >
        <GoogleMark />
      ...
      {!enabled ? (
        <p className="text-xs text-muted-foreground">
          Google OAuth no está configurado (GOOGLE_OAUTH_CLIENT_ID / SECRET). En producción es el
          único método de acceso.
        </p>
      ) : null}
    </form>
  )
